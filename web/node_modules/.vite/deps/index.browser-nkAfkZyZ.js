//#region ../node_modules/@solana/errors/dist/index.browser.mjs
...
function unwrapSimulationError(error) {
	...
	if (isSolanaError(error) && !!error.cause && simulationCodes.includes(error.context.__code)) return error.cause;
	return error;
}
//#endregion
//#region ../node_modules/@solana/codecs-core/dist/index.browser.mjs
var mergeBytes = (byteArrays) => {
	const nonEmptyByteArrays = byteArrays.filter((arr) => arr.length);
	if (nonEmptyByteArrays.length === 0) return byteArrays.length ? byteArrays[0] : /* @__PURE__ */ new Uint8Array();
...
function containsBytes(data, bytes, offset) {
	return bytesEqual((offset === 0 || offset <= -data.byteLength) && data.length === bytes.length ? data : data.slice(offset, offset + bytes.length), bytes);
}
...
function assertIsVariableSize(codec) {
	if (!isVariableSize(codec)) throw new SolanaError(SOLANA_ERROR__CODECS__EXPECTED_VARIABLE_LENGTH);
}
...
function addEncoderSentinel(encoder, sentinel) {
	const write = ((value, bytes, offset) => {
		...
		if (findSentinelIndex(encoderBytes, sentinel) >= 0) throw new SolanaError(SOLANA_ERROR__CODECS__ENCODED_BYTES_MUST_NOT_INCLUDE_SENTINEL, {
	...
	if (isFixedSize(encoder)) return createEncoder({
		...encoder,
...
function addDecoderSentinel(decoder, sentinel) {
	const read = ((bytes, offset) => {
		...
		if (sentinelIndex === -1) throw new SolanaError(SOLANA_ERROR__CODECS__SENTINEL_MISSING_IN_DECODED_BYTES, {
	...
	if (isFixedSize(decoder)) return createDecoder({
		...decoder,
...
function assertByteArrayOffsetIsNotOutOfRange(codecDescription, offset, bytesLength) {
	if (offset < 0 || offset > bytesLength) throw new SolanaError(SOLANA_ERROR__CODECS__OFFSET_OUT_OF_RANGE, {
		bytesLength,
		codecDescription,
		offset
	});
...
function addDecoderSizePrefix(decoder, prefix) {
	...
	if (isFixedSize(prefix) && isFixedSize(decoder)) return createDecoder({
		...decoder,
...
function createDecoderThatConsumesEntireByteArray(decoder) {
	return createDecoder({
		...
		read(bytes, offset) {
			...
			if (bytes.length > newOffset) throw new SolanaError(SOLANA_ERROR__CODECS__EXPECTED_DECODER_TO_CONSUME_ENTIRE_BYTE_ARRAY, {
...
function resizeEncoder(encoder, resize) {
	if (isFixedSize(encoder)) {
		...
		if (fixedSize < 0) throw new SolanaError(SOLANA_ERROR__CODECS__EXPECTED_POSITIVE_BYTE_LENGTH, {
	...
	return createEncoder({
		...
		getSizeFromValue: (value) => {
			...
			if (newSize < 0) throw new SolanaError(SOLANA_ERROR__CODECS__EXPECTED_POSITIVE_BYTE_LENGTH, {
				bytesLength: newSize,
				codecDescription: "resizeEncoder"
			});
			return newSize;
...
//#endregion
//#region ../node_modules/@solana/codecs-strings/dist/index.browser.mjs
...
var getBaseXDecoder$1 = (alphabet4) => {
	return createDecoder({ read(rawBytes, offset) {
		...
		return [leadingZeroes + getBaseXFromBigInt$1(bytes.slice(trailIndex).reduce((sum, byte) => sum * 256n + BigInt(byte), 0n), alphabet4), rawBytes.length];
	} });
};
var getBaseXCodec = (alphabet4) => combineCodec(getBaseXEncoder$1(alphabet4), getBaseXDecoder$1(alphabet4));
...
function getBaseXFromBigInt$1(value, alphabet4) {
	const base = BigInt(alphabet4.length);
	const tailChars = [];
	while (value > 0n) {
...
var getBase10Encoder = () => getBaseXEncoder$1(alphabet);
var getBase10Decoder = () => getBaseXDecoder$1(alphabet);
...
var getBase16Encoder = () => createEncoder({
	...
	write(value, bytes, offset) {
		...
		const al = len / 2;
		if (len === 1) {
			const n = charCodeToBase16(value.charCodeAt(0));
			if (n === void 0) throw new SolanaError(SOLANA_ERROR__CODECS__INVALID_STRING_FOR_BASE, {
		...
		for (let i = 0, j = 0; i < al; i++) {
			...
			if (n1 === void 0 || n2 === void 0 && !Number.isNaN(c2)) throw new SolanaError(SOLANA_ERROR__CODECS__INVALID_STRING_FOR_BASE, {
				...INVALID_STRING_ERROR_BASE_CONFIG,
				value
			});
...
var getBase16Decoder$1 = () => createDecoder({ read(bytes, offset) {
	return [bytes.slice(offset).reduce((str, byte) => str + byte.toString(16).padStart(2, "0"), ""), bytes.length];
} });
var getBase16Codec = () => combineCodec(getBase16Encoder(), getBase16Decoder$1());
...
var getBase58Decoder$1 = () => getBaseXDecoder$1(alphabet2$1);
var getBase58Codec = () => getBaseXCodec(alphabet2$1);
var getBaseXResliceEncoder = (alphabet4, bits) => createEncoder({
	getSizeFromValue: (value) => Math.floor(value.length * bits / 8),
	write(value, bytes, offset) {
		assertValidBaseString$1(alphabet4, value);
		if (value === "") return offset;
		const reslicedBytes = reslice([...value].map((c) => alphabet4.indexOf(c)), bits, 8, false);
		bytes.set(reslicedBytes, offset);
		return reslicedBytes.length + offset;
	}
});
var getBaseXResliceDecoder = (alphabet4, bits) => createDecoder({ read(rawBytes, offset = 0) {
	const bytes = offset === 0 || offset <= -rawBytes.byteLength ? rawBytes : rawBytes.slice(offset);
	if (bytes.length === 0) return ["", rawBytes.length];
	return [reslice([...bytes], 8, bits, true).map((i) => alphabet4[i]).join(""), rawBytes.length];
} });
...
var e$1 = globalThis.TextDecoder;
var o$6 = globalThis.TextEncoder;
var getUtf8Encoder = () => {
	let textEncoder;
	return createEncoder({
		getSizeFromValue: (value) => (textEncoder ||= new o$6()).encode(value).length,
		write: (value, bytes, offset) => {
			const bytesToAdd = (textEncoder ||= new o$6()).encode(value);
			bytes.set(bytesToAdd, offset);
			return offset + bytesToAdd.length;
		}
	});
...
var getUtf8Decoder = () => {
	...
	return createDecoder({ read(bytes, offset) {
		return [removeNullCharacters((textDecoder ||= new e$1()).decode(bytes.slice(offset))), bytes.length];
	} });
};
var getUtf8Codec = () => combineCodec(getUtf8Encoder(), getUtf8Decoder());
//#endregion
//#region ../node_modules/@solana/accounts/dist/index.browser.mjs
...
function decodeAccount(encodedAccount, decoder) {
	try {
		...
		throw new SolanaError(SOLANA_ERROR__ACCOUNTS__FAILED_TO_DECODE_ACCOUNT, { address: encodedAccount.address });
	}
}
...
function assertAccountDecoded(account) {
	if (accountExists(account) && account.data instanceof Uint8Array) throw new SolanaError(SOLANA_ERROR__ACCOUNTS__EXPECTED_DECODED_ACCOUNT, { address: account.address });
}
function assertAccountsDecoded(accounts) {
	const encoded = accounts.filter((a) => accountExists(a) && a.data instanceof Uint8Array);
	if (encoded.length > 0) throw new SolanaError(SOLANA_ERROR__ACCOUNTS__EXPECTED_ALL_ACCOUNTS_TO_BE_DECODED, { addresses: encoded.map((a) => a.address) });
}
...
function parseBase58RpcAccount(address, rpcAccount) {
	...
	const data = getBase58Encoder$1().encode(typeof rpcAccount.data === "string" ? rpcAccount.data : rpcAccount.data[0]);
	return Object.freeze({
		...parseBaseAccount(rpcAccount),
		address,
		data,
...
function parseJsonRpcAccount(address, rpcAccount) {
	...
	if (rpcAccount.data.program || rpcAccount.data.parsed.type) data.parsedAccountMeta = {
...
async function fetchEncodedAccount(rpc, address, config = {}) {
	...
	return parseBase64RpcAccount(address, (await rpc.getAccountInfo(address, {
		...rpcConfig,
		encoding: "base64"
	}).send({ abortSignal })).value);
}
...
async function fetchEncodedAccounts(rpc, addresses, config = {}) {
	const { abortSignal, ...rpcConfig } = config;
	return (await rpc.getMultipleAccounts(addresses, {
		...rpcConfig,
		encoding: "base64"
	}).send({ abortSignal })).value.map((account, index) => parseBase64RpcAccount(addresses[index], account));
}
async function fetchJsonParsedAccounts(rpc, addresses, config = {}) {
	...
	return (await rpc.getMultipleAccounts(addresses, {
		...rpcConfig,
		encoding: "jsonParsed"
	}).send({ abortSignal })).value.map((account, index) => {
...
function assertAccountExists(account) {
	if (!account.exists) throw new SolanaError(SOLANA_ERROR__ACCOUNTS__ACCOUNT_NOT_FOUND, { address: account.address });
}
function assertAccountsExist(accounts) {
	const missingAccounts = accounts.filter((a) => !a.exists);
	if (missingAccounts.length > 0) throw new SolanaError(SOLANA_ERROR__ACCOUNTS__ONE_OR_MORE_ACCOUNTS_NOT_FOUND, { addresses: missingAccounts.map((a) => a.address) });
}
//#endregion
//#region ../node_modules/@solana/assertions/dist/index.browser.mjs
...
function assertIsSecureContext() {
	if (!globalThis.isSecureContext) throw new SolanaError(SOLANA_ERROR__SUBTLE_CRYPTO__DISALLOWED_IN_INSECURE_CONTEXT);
}
...
async function assertKeyGenerationIsAvailable() {
	assertIsSecureContext();
	if (typeof globalThis.crypto === "undefined" || typeof globalThis.crypto.subtle?.generateKey !== "function") throw new SolanaError(SOLANA_ERROR__SUBTLE_CRYPTO__GENERATE_FUNCTION_UNIMPLEMENTED);
	if (!await isEd25519CurveSupported(globalThis.crypto.subtle)) throw new SolanaError(SOLANA_ERROR__SUBTLE_CRYPTO__ED25519_ALGORITHM_UNIMPLEMENTED);
}
...
//#endregion
//#region ../node_modules/@solana/addresses/dist/index.browser.mjs
...
function getMemoizedBase58Decoder() {
	if (!memoizedBase58Decoder) memoizedBase58Decoder = getBase58Decoder$1();
...
function isOffCurveAddress(putativeOffCurveAddress) {
	return compressedPointBytesAreOnCurve(getAddressCodec().encode(putativeOffCurveAddress)) === false;
}
function assertIsOffCurveAddress(putativeOffCurveAddress) {
	if (!isOffCurveAddress(putativeOffCurveAddress)) throw new SolanaError(SOLANA_ERROR__ADDRESSES__INVALID_OFF_CURVE_ADDRESS);
}
...
function assertIsProgramDerivedAddress(value) {
	if (!(Array.isArray(value) && value.length === 2 && typeof value[0] === "string" && typeof value[1] === "number")) throw new SolanaError(SOLANA_ERROR__ADDRESSES__MALFORMED_PDA);
	if (value[1] < 0 || value[1] > 255) throw new SolanaError(SOLANA_ERROR__ADDRESSES__PDA_BUMP_SEED_OUT_OF_RANGE, { bump: value[1] });
...
async function createAddressWithSeed({ baseAddress, programAddress, seed }) {
	...
	if (seedBytes.byteLength > MAX_SEED_LENGTH) throw new SolanaError(SOLANA_ERROR__ADDRESSES__MAX_PDA_SEED_LENGTH_EXCEEDED, {
		actual: seedBytes.byteLength,
		index: 0,
		maxSeedLength: MAX_SEED_LENGTH
	});
	const programAddressBytes = encode(programAddress);
	if (programAddressBytes.length >= PDA_MARKER_BYTES.length && bytesEqual(programAddressBytes.slice(-PDA_MARKER_BYTES.length), new Uint8Array(PDA_MARKER_BYTES))) throw new SolanaError(SOLANA_ERROR__ADDRESSES__PDA_ENDS_WITH_PDA_MARKER);
	const addressBytesBuffer = await crypto.subtle.digest("SHA-256", new Uint8Array([
		...encode(baseAddress),
		...seedBytes,
		...programAddressBytes
	]));
	return decode(new Uint8Array(addressBytesBuffer));
}
...
//#endregion
//#region ../node_modules/@solana/codecs-numbers/dist/index.browser.mjs
...
//#endregion
//#region ../node_modules/@solana/codecs-data-structures/dist/index.browser.mjs
...
function getArrayDecoder(item, config = {}) {
	...
	return createDecoder({
		...
		read: (bytes, offset) => {
			const array = [];
			if (typeof size === "object" && bytes.slice(offset).length === 0) return [array, offset];
			if (size === "remainder") {
...
function getBitArrayEncoder(size, config = {}) {
	const backward = (typeof config === "boolean" ? { backward: config } : config).backward ?? false;
	return createEncoder({
		fixedSize: size,
		write(value, bytes, offset) {
			const bytesToAdd = [];
...
var getBase16Decoder = () => createDecoder({ read(bytes, offset) {
	return [bytes.slice(offset).reduce((str, byte) => str + byte.toString(16).padStart(2, "0"), ""), bytes.length];
} });
...
function getConstantDecoder(constant) {
	return createDecoder({
		...
		read: (bytes, offset) => {
			...
			if (!containsBytes(bytes, constant, offset)) throw new SolanaError(SOLANA_ERROR__CODECS__INVALID_CONSTANT, {
				constant,
				data: bytes,
...
function buildFromEntries(entries) {
	return {
		build() {
			...
			const fixedSize = staticDecoders.every((d) => d !== void 0) ? sumCodecSizes(staticDecoders.map(getFixedSize)) : null;
			const read = (bytes, offset) => {
				const decoded = {};
				for (const { name, resolveDecoder } of entries) {
					const [value, newOffset] = resolveDecoder(decoded).read(bytes, offset);
					decoded[name] = value;
					offset = newOffset;
				}
				return [decoded, offset];
			};
			return fixedSize === null ? createDecoder({ read }) : createDecoder({
				fixedSize,
				read
			});
		},
		field(name, decoderOrFactory) {
			...
			const nextEntry = {
				name,
				resolveDecoder: isFactory ? (fields) => decoderOrFactory(Object.freeze({ ...fields })) : () => decoderOrFactory,
...
function getTupleCodec(items, config) {
	return combineCodec(getTupleEncoder(items, config), getTupleDecoder(items));
}
...
function getUnionDecoder(variants, getIndexFromBytes) {
	...
	if (fixedSize !== null) return createDecoder({
		fixedSize,
		read
	});
	const maxSize = getUnionMaxSize(variants);
...
function getVariantDiscriminator(variants, discriminatorValue) {
	...
	if (discriminator < 0) throw new SolanaError(SOLANA_ERROR__CODECS__INVALID_DISCRIMINATED_UNION_VARIANT, {
...
function getEnumStats(constructor) {
	...
	return {
		...
		enumValues,
		numericalValues,
		stringValues: [.../* @__PURE__ */ new Set([...enumKeys, ...enumValues.filter((v) => typeof v === "string")])]
	};
}
...
function getEnumEncoder(constructor, config = {}) {
	...
	if (useValuesAsDiscriminators && enumValues.some((value) => typeof value === "string")) throw new SolanaError(SOLANA_ERROR__CODECS__CANNOT_USE_LEXICAL_VALUES_AS_ENUM_DISCRIMINATORS, { stringValues: enumValues.filter((v) => typeof v === "string") });
	return transformEncoder(prefix, (variant) => {
		...
		if (index < 0) throw new SolanaError(SOLANA_ERROR__CODECS__INVALID_ENUM_VARIANT, {
...
function getLiteralUnionEncoder(variants, config = {}) {
	return transformEncoder(config.size ?? getU8Encoder(), (variant) => {
		const index = variants.indexOf(variant);
		if (index < 0) throw new SolanaError(SOLANA_ERROR__CODECS__INVALID_LITERAL_UNION_VARIANT, {
			value: variant,
			variants
		});
		return index;
...
function getLiteralUnionDecoder(variants, config = {}) {
	return transformDecoder(config.size ?? getU8Decoder(), (index) => {
		if (index < 0 || index >= variants.length) throw new SolanaError(SOLANA_ERROR__CODECS__LITERAL_UNION_DISCRIMINATOR_OUT_OF_RANGE, {
			discriminator: index,
			maxRange: variants.length - 1,
			minRange: 0
		});
...
function getNullableDecoder(item, config = {}) {
	const prefix = (() => {
		if (config.prefix === null) return transformDecoder(getUnitDecoder(), () => false);
	...
	const noneValue = (() => {
		...
		if (!config.noneValue) return getUnitDecoder();
	...
	return getUnionDecoder([transformDecoder(getTupleDecoder([prefix, noneValue]), () => null), transformDecoder(getTupleDecoder([prefix, item]), ([, value]) => value)], (bytes, offset) => {
		...
		if (config.prefix === null && config.noneValue != null) return containsBytes(bytes, config.noneValue === "zeroes" ? new Uint8Array(noneValue.fixedSize).fill(0) : config.noneValue, offset) ? 0 : 1;
...
function getPatternMatchDecoder(patterns) {
	return getUnionDecoder(patterns.map(([, decoder]) => decoder), (value) => {
		const index = patterns.findIndex(([predicate]) => predicate(value));
		if (index === -1) throw new SolanaError(SOLANA_ERROR__CODECS__INVALID_PATTERN_MATCH_BYTES, { bytes: value });
		return index;
	});
}
function getPatternMatchCodec(patterns) {
	return combineCodec(getPatternMatchEncoder(patterns.map(([valuePredicate, , codec]) => [valuePredicate, codec])), getPatternMatchDecoder(patterns.map(([, bytesPredicate, codec]) => [bytesPredicate, codec])));
}
...
function getPredicateCodec(encodePredicate, decodePredicate, ifTrue, ifFalse) {
	return getUnionCodec([ifTrue, ifFalse], (value) => encodePredicate(value) ? 0 : 1, (value) => decodePredicate(value) ? 0 : 1);
}
...
//#endregion
//#region ../node_modules/@solana/fixed-points/dist/index.browser.mjs
function getRawRange(signedness, totalBits) {
	...
	return {
		max: (1n << BigInt(totalBits)) - 1n,
		min: 0n
	};
}
function assertValidTotalBits(kind, totalBits) {
	if (typeof totalBits !== "number" || !Number.isInteger(totalBits) || totalBits <= 0) throw new SolanaError(SOLANA_ERROR__FIXED_POINTS__INVALID_TOTAL_BITS, {
		kind,
		totalBits
	});
}
function assertValidFractionalBits(fractionalBits) {
	if (typeof fractionalBits !== "number" || !Number.isInteger(fractionalBits) || fractionalBits < 0) throw new SolanaError(SOLANA_ERROR__FIXED_POINTS__INVALID_FRACTIONAL_BITS, { fractionalBits });
}
function assertValidDecimals(decimals) {
	if (typeof decimals !== "number" || !Number.isInteger(decimals) || decimals < 0) throw new SolanaError(SOLANA_ERROR__FIXED_POINTS__INVALID_DECIMALS, { decimals });
}
function assertFractionalBitsFitInTotalBits(fractionalBits, totalBits) {
	if (fractionalBits > totalBits) throw new SolanaError(SOLANA_ERROR__FIXED_POINTS__FRACTIONAL_BITS_EXCEED_TOTAL_BITS, {
		fractionalBits,
		totalBits
	});
}
function assertTotalBitsIsByteAligned(kind, totalBits) {
	if (totalBits % 8 !== 0) throw new SolanaError(SOLANA_ERROR__FIXED_POINTS__TOTAL_BITS_NOT_BYTE_ALIGNED, {
		kind,
		totalBits
	});
}
function assertRawFitsInRange(kind, signedness, totalBits, raw) {
	...
	if (raw < min || raw > max) throw new SolanaError(SOLANA_ERROR__FIXED_POINTS__VALUE_OUT_OF_RANGE, {
		kind,
		max,
		min,
		raw,
...
function assertShapeMatches(operation, actual, expected) {
	if (!((actual.signedness === "signed" || actual.signedness === "unsigned") && Number.isInteger(actual.totalBits) && actual.totalBits > 0 && Number.isInteger(actual.scale) && actual.scale >= 0) || actual.kind !== expected.kind || expected.signedness !== void 0 && actual.signedness !== expected.signedness || expected.totalBits !== void 0 && actual.totalBits !== expected.totalBits || expected.scale !== void 0 && actual.scale !== expected.scale) throw new SolanaError(SOLANA_ERROR__FIXED_POINTS__SHAPE_MISMATCH, {
...
function assertNoArithmeticOverflow(kind, operation, signedness, totalBits, result) {
	const { max, min } = getRawRange(signedness, totalBits);
	if (result < min || result > max) throw new SolanaError(SOLANA_ERROR__FIXED_POINTS__ARITHMETIC_OVERFLOW, {
		kind,
		max,
		min,
		operation,
...
function assertNoDivisionByZero(kind, signedness, totalBits, denominator) {
	if (denominator === 0n) throw new SolanaError(SOLANA_ERROR__FIXED_POINTS__DIVISION_BY_ZERO, {
		kind,
		signedness,
		totalBits
	});
...
function roundDivision(kind, operation, numerator, denominator, mode) {
	...
	if (mode === "strict") throw new SolanaError(SOLANA_ERROR__FIXED_POINTS__STRICT_MODE_PRECISION_LOSS, {
		kind,
		operation
	});
	const sameSign = numerator < 0n === denominator < 0n;
	if (mode === "trunc") return quotient;
	if (mode === "floor") return sameSign ? quotient : quotient - 1n;
	if (mode === "ceil") return sameSign ? quotient + 1n : quotient;
	if ((remainder < 0n ? -remainder : remainder) * 2n < (denominator < 0n ? -denominator : denominator)) return quotient;
...
function multiplyBinaryFixedPoint(a, b, rounding = "strict") {
	let result;
	if (typeof b === "bigint") result = a.raw * b;
	else {
...
function readRawBigInt(bytes, offset, byteSize, signedness, littleEndian) {
	...
	if (signedness === "signed") {
		...
		if ((unsigned & signBit) !== 0n) return unsigned - (1n << BigInt(byteSize * 8));
	}
	return unsigned;
}
...
function getBinaryFixedPointCodec(signedness, totalBits, fractionalBits, config = {}) {
	return combineCodec(getBinaryFixedPointEncoder(signedness, totalBits, fractionalBits, config), getBinaryFixedPointDecoder(signedness, totalBits, fractionalBits, config));
}
...
function binaryFixedPointToBase10(value) {
	...
	return {
		decimals,
		raw: decimals === 0 ? value.raw : value.raw * 5n ** BigInt(decimals)
	};
}
function toUnsignedBinaryFixedPoint(value) {
	if (value.signedness === "unsigned") return value;
...
function toSignedBinaryFixedPoint(value) {
	if (value.signedness === "signed") return value;
...
function rescaleBinaryFixedPoint(value, newTotalBits, newFractionalBits, rounding = "strict") {
	...
	if (value.totalBits === newTotalBits && value.fractionalBits === newFractionalBits) return value;
	let result;
	if (newFractionalBits === value.fractionalBits) result = value.raw;
	else if (newFractionalBits > value.fractionalBits) result = value.raw << BigInt(newFractionalBits - value.fractionalBits);
	else result = roundDivision("binaryFixedPoint", "rescale", value.raw, 1n << BigInt(value.fractionalBits - newFractionalBits), rounding);
...
function parseDecimalString(kind, input) {
	if (typeof input !== "string" || !/^-?(?:\d+\.?\d*|\.\d+)$/.test(input)) throw new SolanaError(SOLANA_ERROR__FIXED_POINTS__INVALID_STRING, {
		input: String(input),
		kind
	});
...
function binaryFixedPoint(signedness, totalBits, fractionalBits) {
	...
	return (input, rounding = "strict") => {
		...
		return createBinaryFixedPoint(signedness, totalBits, fractionalBits, parsed.decimals === 0 ? scaledRaw : roundDivision("binaryFixedPoint", "fromString", scaledRaw, 10n ** BigInt(parsed.decimals), rounding));
	};
}
...
function ratioBinaryFixedPoint(signedness, totalBits, fractionalBits) {
	...
	return (numerator, denominator, rounding = "strict") => {
		if (denominator === 0n) throw new SolanaError(SOLANA_ERROR__FIXED_POINTS__INVALID_ZERO_DENOMINATOR_RATIO, {
			denominator,
			kind: "binaryFixedPoint",
			numerator
		});
		return createBinaryFixedPoint(signedness, totalBits, fractionalBits, roundDivision("binaryFixedPoint", "fromRatio", numerator * (1n << BigInt(fractionalBits)), denominator, rounding));
	};
}
function applyDecimalsOption(kind, raw, currentDecimals, options) {
	const targetDecimals = options?.decimals;
	if (targetDecimals === void 0 || targetDecimals === currentDecimals) return {
		decimals: currentDecimals,
		raw
	};
	if (targetDecimals > currentDecimals) return {
		decimals: targetDecimals,
		raw: raw * 10n ** BigInt(targetDecimals - currentDecimals)
	};
	return {
		decimals: targetDecimals,
		raw: roundDivision(kind, "toString", raw, 10n ** BigInt(currentDecimals - targetDecimals), options?.rounding ?? "strict")
	};
}
function formatScaledBigint(raw, decimals, padTrailingZeros) {
	if (decimals === 0) return raw.toString();
	const isNegative = raw < 0n;
	const padded = (isNegative ? -raw : raw).toString().padStart(decimals + 1, "0");
	const integerPart = padded.slice(0, -decimals);
	let fractionalPart = padded.slice(-decimals);
	if (!padTrailingZeros) fractionalPart = fractionalPart.replace(/0+$/, "");
	const sign = isNegative ? "-" : "";
	if (fractionalPart.length === 0) return `${sign}${integerPart}`;
...
function binaryFixedPointToNumber(value) {
	...
	if (fractionalBits === 0) return Number(raw);
...
function assertIsBinaryFixedPoint(value, signedness, totalBits, fractionalBits) {
	...
	assertShapeMatches("assertIsBinaryFixedPoint", actual, {
		kind: "binaryFixedPoint",
		scale: fractionalBits,
		scaleLabel: "fractional bits",
		signedness,
...
function getDecimalFixedPointCodec(signedness, totalBits, decimals, config = {}) {
	return combineCodec(getDecimalFixedPointEncoder(signedness, totalBits, decimals, config), getDecimalFixedPointDecoder(signedness, totalBits, decimals, config));
}
...
function rescaleDecimalFixedPoint(value, newTotalBits, newDecimals, rounding = "strict") {
	...
	if (value.totalBits === newTotalBits && value.decimals === newDecimals) return value;
	let result;
	if (newDecimals === value.decimals) result = value.raw;
	else if (newDecimals > value.decimals) result = value.raw * 10n ** BigInt(newDecimals - value.decimals);
	else result = roundDivision("decimalFixedPoint", "rescale", value.raw, 10n ** BigInt(value.decimals - newDecimals), rounding);
...
function decimalFixedPoint(signedness, totalBits, decimals) {
	...
	return (input, rounding = "strict") => {
		...
		return createDecimalFixedPoint(signedness, totalBits, decimals, parsed.decimals <= decimals ? parsed.raw * 10n ** BigInt(decimals - parsed.decimals) : roundDivision("decimalFixedPoint", "fromString", parsed.raw, 10n ** BigInt(parsed.decimals - decimals), rounding));
	};
}
...
function ratioDecimalFixedPoint(signedness, totalBits, decimals) {
	...
	return (numerator, denominator, rounding = "strict") => {
		...
		return createDecimalFixedPoint(signedness, totalBits, decimals, roundDivision("decimalFixedPoint", "fromRatio", numerator * 10n ** BigInt(decimals), denominator, rounding));
	};
}
...
function assertIsDecimalFixedPoint(value, signedness, totalBits, decimals) {
	const actual = describeShape(value);
	assertShapeMatches("assertIsDecimalFixedPoint", actual, {
		kind: "decimalFixedPoint",
		scale: decimals,
		scaleLabel: "decimals",
		signedness,
...
//#endregion
//#region ../node_modules/@solana/options/dist/index.browser.mjs
var some = (value) => ({
	__option: "Some",
	value
});
...
function getOptionEncoder(item, config = {}) {
	...
	return getUnionEncoder([transformEncoder(getTupleEncoder([prefix, (() => {
		...
		if (!config.noneValue) return getUnitEncoder();
		return getConstantEncoder(config.noneValue);
	})()]), (_value) => [false, void 0]), transformEncoder(getTupleEncoder([prefix, item]), (value) => [true, isOption(value) && isSome(value) ? value.value : value])], (variant) => {
...
//#endregion
//#region ../node_modules/@solana/functional/dist/index.browser.mjs
function pipe(init, ...fns) {
	return fns.reduce((acc, fn) => fn(acc), init);
}
//#endregion
//#region ../node_modules/@solana/instructions/dist/index.browser.mjs
...
function assertIsInstructionForProgram(instruction, programAddress) {
	if (instruction.programAddress !== programAddress) throw new SolanaError(SOLANA_ERROR__INSTRUCTION__PROGRAM_ID_MISMATCH, {
...
function assertIsInstructionWithAccounts(instruction) {
	if (instruction.accounts === void 0) throw new SolanaError(SOLANA_ERROR__INSTRUCTION__EXPECTED_TO_HAVE_ACCOUNTS, {
...
function assertIsInstructionWithData(instruction) {
	if (instruction.data === void 0) throw new SolanaError(SOLANA_ERROR__INSTRUCTION__EXPECTED_TO_HAVE_DATA, {
...
//#endregion
//#region ../node_modules/@solana/rpc-types/dist/index.browser.mjs
...
function assertIsBlockhash(putativeBlockhash) {
	try {
		...
		if (isSolanaError(error, 2800001)) throw new SolanaError(4, error.context);
		if (isSolanaError(error, 28e5)) throw new SolanaError(5, error.context);
		throw error;
	}
}
...
function getCommitmentScore(commitment) {
	switch (commitment) {
		...
		default: throw new SolanaError(SOLANA_ERROR__INVARIANT_VIOLATION__SWITCH_MUST_BE_EXHAUSTIVE, { unexpectedValue: commitment });
	}
}
...
function assertIsLamports(putativeLamports) {
	if (putativeLamports < 0 || putativeLamports > maxU64Value) throw new SolanaError(6);
}
...
function getLamportsEncoder(innerEncoder) {
	return transformEncoder(innerEncoder, (value) => typeof value === "bigint" ? value : value.raw);
}
...
function getSolEncoder() {
	return transformEncoder(getMemoizedU64Encoder2(), (value) => typeof value === "bigint" ? value : value.raw);
}
...
function assertIsStringifiedBigInt(putativeBigInt) {
	try {
		...
		throw new SolanaError(7, { value: putativeBigInt });
	}
}
...
function assertIsStringifiedNumber(putativeNumber) {
	if (Number.isNaN(Number(putativeNumber))) throw new SolanaError(8, { value: putativeNumber });
}
...
function assertIsUnixTimestamp(putativeTimestamp) {
	if (putativeTimestamp < minI64Value || putativeTimestamp > maxI64Value) throw new SolanaError(9, { value: putativeTimestamp });
}
...
//#endregion
//#region ../node_modules/@solana/transaction-messages/dist/index.browser.mjs
...
function assertIsTransactionMessageWithBlockhashLifetime(transactionMessage) {
	if (!isTransactionMessageWithBlockhashLifetime(transactionMessage)) throw new SolanaError(SOLANA_ERROR__TRANSACTION__EXPECTED_BLOCKHASH_LIFETIME);
}
...
function getInstructionEncoder() {
	if (!memoizedGetInstructionEncoder) memoizedGetInstructionEncoder = transformEncoder(getStructEncoder([
		...
		return {
			...
			data: instruction.data ?? /* @__PURE__ */ new Uint8Array(0)
		};
	});
...
function getInstructionDecoder() {
	if (!memoizedGetInstructionDecoder) memoizedGetInstructionDecoder = transformDecoder(getStructDecoder([
		["programAddressIndex", getU8Decoder()],
		["accountIndices", getArrayDecoder(getU8Decoder(), { size: getShortU16Decoder() })],
		["data", addDecoderSizePrefix(getBytesDecoder(), getShortU16Decoder())]
	]), (instruction) => {
...
function getTransactionVersionEncoder() {
	return createEncoder({
		...
		write: (value, bytes, offset) => {
			...
			bytes.set([value | VERSION_FLAG_MASK$1], offset);
			return offset + 1;
		}
	});
}
function getTransactionVersionDecoder() {
	return createDecoder({
		...
		read: (bytes, offset) => {
			...
			if ((firstByte & VERSION_FLAG_MASK$1) === 0) return ["legacy", offset];
			else {
				const version = firstByte ^ VERSION_FLAG_MASK$1;
...
function getMessageDecoder2() {
	return transformDecoder(getStructDecoder([
		...
		if (!addressTableLookups?.length) return {
			...restOfMessage,
			version: 0
		};
		return {
...
function setTransactionMessageConfig(config, transactionMessage) {
	...
	if (isV1ConfigEmpty(mergedConfig)) {
		if (!transactionMessage.config) return transactionMessage;
		const { config: config2, ...rest } = transactionMessage;
		return Object.freeze(rest);
	}
	if (transactionMessage.config && areV1ConfigsEqual(transactionMessage.config, mergedConfig)) return transactionMessage;
	return Object.freeze({
		...transactionMessage,
...
function transactionConfigMaskHasPriorityFee(mask) {
	...
	if (priorityFeeBits === 1 || priorityFeeBits === 2) throw new SolanaError(SOLANA_ERROR__TRANSACTION__INVALID_CONFIG_MASK_PRIORITY_FEE_BITS, { mask });
...
function getCompiledTransactionConfigValueEncoder() {
	return getPatternMatchEncoder([[(value) => value.kind === "u32", transformEncoder(getStructEncoder([["value", getU32Encoder()]]), (value) => value)], [(value) => value.kind === "u64", transformEncoder(getStructEncoder([["value", getU64Encoder()]]), (value) => value)]]);
}
...
function getCompiledTransactionConfigValuesDecoder(mask) {
	...
	const u32Decoder = transformDecoder(getU32Decoder(), (value) => ({
		kind: "u32",
		value
	}));
	const u64Decoder = transformDecoder(getU64Decoder(), (value) => ({
		kind: "u64",
		value
	}));
	const unitDecoder = getUnitDecoder();
	return transformDecoder(getTupleDecoder([
...
function getInstructionPayloadDecoder(instructionHeader) {
	return getStructDecoder([["instructionAccountIndices", getArrayDecoder(getU8Decoder(), { size: instructionHeader.numInstructionAccounts })], ["instructionData", fixDecoderSize(getBytesDecoder(), instructionHeader.numInstructionDataBytes)]]);
}
...
function getMessageDecoder3() {
	return createDecoder({ read(bytes, offset) {
		...
		const [staticAccounts, afterAddresses] = getArrayDecoder(getAddressDecoder(), { size: numStaticAccounts }).read(bytes, nextOffset);
		...
		const [instructionHeaders, afterHeaders] = getArrayDecoder(getInstructionHeaderDecoder(), { size: numInstructions }).read(bytes, nextOffset);
...
function compileTransactionMessage(transactionMessage) {
	...
	if (numAccounts > 64) throw new SolanaError(SOLANA_ERROR__TRANSACTION__TOO_MANY_ACCOUNT_ADDRESSES, {
		actualCount: numAccounts,
		maxAllowed: 64
	});
	const numSigners = orderedAccounts.filter((account) => isSignerRole(account.role)).length;
	if (numSigners > 12) throw new SolanaError(SOLANA_ERROR__TRANSACTION__TOO_MANY_SIGNER_ADDRESSES, {
		actualCount: numSigners,
		maxAllowed: 12
	});
	const numInstructions = transactionMessage.instructions.length;
	if (numInstructions > 64) throw new SolanaError(SOLANA_ERROR__TRANSACTION__TOO_MANY_INSTRUCTIONS, {
	...
	for (let i = 0; i < transactionMessage.instructions.length; i++) {
		...
		if (numAccountsInInstruction > 255) throw new SolanaError(SOLANA_ERROR__TRANSACTION__TOO_MANY_ACCOUNTS_IN_INSTRUCTION, {
...
function getInstructionPayload(instruction, accountIndex) {
	return {
		...
		instructionData: instruction.data ?? /* @__PURE__ */ new Uint8Array()
	};
}
...
function findAddressInLookupTables(address, role, addressesByLookupTableAddress) {
	for (const [lookupTableAddress, addresses] of Object.entries(addressesByLookupTableAddress)) for (let i = 0; i < addresses.length; i++) if (address === addresses[i]) return {
		address,
		addressIndex: i,
		lookupTableAddress,
		role
...
function getSetLoadedAccountsDataSizeLimitInstruction(limit) {
	return getComputeBudgetInstruction(SET_LOADED_ACCOUNTS_DATA_SIZE_LIMIT_DISCRIMINATOR, getU32Encoder().encode(limit));
}
...
function convertInstruction(instruction, accountMetas) {
	...
	if (!programAddress) throw new SolanaError(SOLANA_ERROR__TRANSACTION__FAILED_TO_DECOMPILE_INSTRUCTION_PROGRAM_ADDRESS_NOT_FOUND, { index: instruction.programAddressIndex });
...
function getFeePayer(staticAccounts) {
	...
	if (!feePayer) throw new SolanaError(SOLANA_ERROR__TRANSACTION__FAILED_TO_DECOMPILE_FEE_PAYER_MISSING);
	return feePayer;
}
var RECENT_BLOCKHASHES_SYSVAR_ADDRESS = "SysvarRecentB1ockHashes11111111111111111111";
var SYSTEM_PROGRAM_ADDRESS$1 = "11111111111111111111111111111111";
function createAdvanceNonceAccountInstruction(nonceAccountAddress, nonceAuthorityAddress) {
	return {
		...
		programAddress: SYSTEM_PROGRAM_ADDRESS$1
	};
}
function isAdvanceNonceAccountInstruction(instruction) {
	return instruction.programAddress === SYSTEM_PROGRAM_ADDRESS$1 && instruction.data != null && isAdvanceNonceAccountInstructionData$1(instruction.data) && instruction.accounts?.length === 3 && instruction.accounts[0].address != null && instruction.accounts[0].role === AccountRole.WRITABLE && instruction.accounts[1].address === RECENT_BLOCKHASHES_SYSVAR_ADDRESS && instruction.accounts[1].role === AccountRole.READONLY && instruction.accounts[2].address != null && isSignerRole(instruction.accounts[2].role);
}
function isAdvanceNonceAccountInstructionData$1(data) {
	return data.byteLength === 4 && data[0] === 4 && data[1] === 0 && data[2] === 0 && data[3] === 0;
}
function getLifetimeConstraint(messageLifetimeToken, instructions, lastValidBlockHeight) {
	const firstInstruction = instructions[0];
	if (!firstInstruction || !isAdvanceNonceAccountInstruction(firstInstruction)) return {
...
function assertIsTransactionMessageWithDurableNonceLifetime(transactionMessage) {
	if (!isTransactionMessageWithDurableNonceLifetime(transactionMessage)) throw new SolanaError(SOLANA_ERROR__TRANSACTION__EXPECTED_NONCE_LIFETIME);
}
...
function setTransactionMessageLifetimeUsingDurableNonce({ nonce, nonceAccountAddress, nonceAuthorityAddress }, transactionMessage) {
	...
	if (firstInstruction && isAdvanceNonceAccountInstruction(firstInstruction)) if (isAdvanceNonceAccountInstructionForNonce(firstInstruction, nonceAccountAddress, nonceAuthorityAddress)) if (isTransactionMessageWithDurableNonceLifetime(transactionMessage) && transactionMessage.lifetimeConstraint.nonce === nonce) return transactionMessage;
	else newInstructions = [firstInstruction, ...transactionMessage.instructions.slice(1)];
	else newInstructions = [Object.freeze(createAdvanceNonceAccountInstruction(nonceAccountAddress, nonceAuthorityAddress)), ...transactionMessage.instructions.slice(1)];
	else newInstructions = [Object.freeze(createAdvanceNonceAccountInstruction(nonceAccountAddress, nonceAuthorityAddress)), ...transactionMessage.instructions];
	return Object.freeze({
		...transactionMessage,
...
function getAddressLookupMetas(compiledAddressTableLookups, addressesByLookupTableAddress) {
	const missing = compiledAddressTableLookups.map((l) => l.lookupTableAddress).filter((a) => addressesByLookupTableAddress[a] === void 0);
	if (missing.length > 0) throw new SolanaError(SOLANA_ERROR__TRANSACTION__FAILED_TO_DECOMPILE_ADDRESS_LOOKUP_TABLE_CONTENTS_MISSING, { lookupTableAddresses: missing });
	const readOnlyMetas = [];
	const writableMetas = [];
	for (const lookup of compiledAddressTableLookups) {
		...
		if (highestIndex >= addresses.length) throw new SolanaError(SOLANA_ERROR__TRANSACTION__FAILED_TO_DECOMPILE_ADDRESS_LOOKUP_TABLE_INDEX_OUT_OF_RANGE, {
...
function decompileTransactionMessage2(compiledTransactionMessage, config) {
	...
	const lifetimeConstraint = getLifetimeConstraint(compiledTransactionMessage.lifetimeToken, instructions, config?.lastValidBlockHeight);
	return pipe(createTransactionMessage({ version: 0 }), (m) => setTransactionMessageFeePayer(feePayer, m), (m) => appendTransactionMessageInstructions(instructions, m), (m) => "blockhash" in lifetimeConstraint ? setTransactionMessageLifetimeUsingBlockhash(lifetimeConstraint, m) : setTransactionMessageLifetimeUsingDurableNonce(lifetimeConstraint, m));
}
function decompileTransactionConfig(configMask, configValues) {
	...
	].reduce(([acc, index], [name, kind, predicate]) => {
		if (!predicate(configMask)) return [acc, index];
		const configValue = configValues[index];
		if (configValue.kind !== kind) throw new SolanaError(SOLANA_ERROR__TRANSACTION__INVALID_CONFIG_VALUE_KIND, {
...
function decompileInstruction(instructionHeader, instructionPayload, accountMetas) {
	...
	if (!programAddress) throw new SolanaError(SOLANA_ERROR__TRANSACTION__FAILED_TO_DECOMPILE_INSTRUCTION_PROGRAM_ADDRESS_NOT_FOUND, { index: instructionHeader.programAccountIndex });
...
function decompileInstructions(instructionHeaders, instructionPayloads, accountMetas) {
	if (instructionHeaders.length !== instructionPayloads.length) throw new SolanaError(SOLANA_ERROR__TRANSACTION__INSTRUCTION_HEADERS_PAYLOADS_MISMATCH, {
...
function decompileTransactionMessage3(compiledTransactionMessage, config) {
	...
	const lifetimeConstraint = getLifetimeConstraint(compiledTransactionMessage.lifetimeToken, instructions, config?.lastValidBlockHeight);
	return pipe(createTransactionMessage({ version: 1 }), (m) => setTransactionMessageConfig(transactionConfig, m), (m) => setTransactionMessageFeePayer(feePayer, m), (m) => appendTransactionMessageInstructions(instructions, m), (m) => "blockhash" in lifetimeConstraint ? setTransactionMessageLifetimeUsingBlockhash(lifetimeConstraint, m) : setTransactionMessageLifetimeUsingDurableNonce(lifetimeConstraint, m));
}
function decompileTransactionMessage4(compiledTransactionMessage, config) {
	const version = compiledTransactionMessage.version;
	if (version === "legacy") return decompileTransactionMessage(compiledTransactionMessage, config);
	else if (version === 0) return decompileTransactionMessage2(compiledTransactionMessage, config);
	else if (version === 1) return decompileTransactionMessage3(compiledTransactionMessage, config);
	else throw new SolanaError(SOLANA_ERROR__TRANSACTION__VERSION_NUMBER_NOT_SUPPORTED, { version });
}
function getTransactionMessageHeapSize(transactionMessage) {
	switch (transactionMessage.version) {
		case 1: return transactionMessage.config?.heapSize;
		default: return getTransactionMessageHeapSizeUsingInstruction(transactionMessage);
	}
}
function getTransactionMessageHeapSizeUsingInstruction(transactionMessage) {
	const existingInstruction = transactionMessage.instructions.find(isRequestHeapFrameInstruction);
...
function setTransactionMessageHeapSize(heapSize, transactionMessage) {
	switch (transactionMessage.version) {
		case 1: return setTransactionMessageHeapSizeUsingConfig(heapSize, transactionMessage);
		default: return setTransactionMessageHeapSizeUsingInstruction(heapSize, transactionMessage);
	}
}
...
function setTransactionMessageHeapSizeUsingInstruction(heapSize, transactionMessage) {
	...
	if (heapSize === void 0) return existingIndex === -1 ? transactionMessage : removeTransactionMessageInstruction(existingIndex, transactionMessage);
	if (getTransactionMessageHeapSize(transactionMessage) === heapSize) return transactionMessage;
...
function getTransactionMessageLoadedAccountsDataSizeLimit(transactionMessage) {
	switch (transactionMessage.version) {
		case 1: return transactionMessage.config?.loadedAccountsDataSizeLimit;
		default: return getTransactionMessageLoadedAccountsDataSizeLimitUsingInstruction(transactionMessage);
	}
}
function getTransactionMessageLoadedAccountsDataSizeLimitUsingInstruction(transactionMessage) {
	const existingInstruction = transactionMessage.instructions.find(isSetLoadedAccountsDataSizeLimitInstruction);
...
function setTransactionMessageLoadedAccountsDataSizeLimit(loadedAccountsDataSizeLimit, transactionMessage) {
	switch (transactionMessage.version) {
		case 1: return setTransactionMessageLoadedAccountsDataSizeLimitUsingConfig(loadedAccountsDataSizeLimit, transactionMessage);
		default: return setTransactionMessageLoadedAccountsDataSizeLimitUsingInstruction(loadedAccountsDataSizeLimit, transactionMessage);
	}
}
...
function setTransactionMessageLoadedAccountsDataSizeLimitUsingInstruction(loadedAccountsDataSizeLimit, transactionMessage) {
	...
	if (loadedAccountsDataSizeLimit === void 0) return existingIndex === -1 ? transactionMessage : removeTransactionMessageInstruction(existingIndex, transactionMessage);
	if (getTransactionMessageLoadedAccountsDataSizeLimit(transactionMessage) === loadedAccountsDataSizeLimit) return transactionMessage;
...
//#endregion
//#region ../node_modules/@solana/promises/dist/index.browser.mjs
...
async function safeRace(contenders) {
	...
	return await new Promise((resolve, reject) => {
		...
		for (const contender of contenders) {
			...
			} else if (record.settled) Promise.resolve(contender).then(resolve, reject);
			else record.deferreds.add(deferred);
		}
	}).finally(() => {
		for (const contender of contenders) if (isObject(contender)) wm.get(contender).deferreds.delete(deferred);
	});
}
function getAbortablePromise(promise, abortSignal) {
	if (!abortSignal) return promise;
	else return safeRace([new Promise((_, reject) => {
		if (abortSignal.aborted) reject(abortSignal.reason);
		else abortSignal.addEventListener("abort", function() {
			reject(this.reason);
		});
	}), promise]);
}
//#endregion
//#region ../node_modules/@solana/keys/dist/index.browser.mjs
...
async function getPublicKeyFromPrivateKey(privateKey, extractable = false) {
	assertKeyExporterIsAvailable();
	if (privateKey.extractable === false) throw new SolanaError(SOLANA_ERROR__SUBTLE_CRYPTO__CANNOT_EXPORT_NON_EXTRACTABLE_KEY, { key: privateKey });
...
function assertIsSignature(putativeSignature) {
	if (!base58Encoder) base58Encoder = getBase58Encoder$1();
	if (putativeSignature.length < 64 || putativeSignature.length > 88) throw new SolanaError(SOLANA_ERROR__KEYS__SIGNATURE_STRING_LENGTH_OUT_OF_RANGE, { actualLength: putativeSignature.length });
	assertIsSignatureBytes(base58Encoder.encode(putativeSignature));
}
function assertIsSignatureBytes(putativeSignatureBytes) {
	const numBytes = putativeSignatureBytes.byteLength;
	if (numBytes !== 64) throw new SolanaError(SOLANA_ERROR__KEYS__INVALID_SIGNATURE_BYTE_LENGTH, { actualLength: numBytes });
}
function isSignature(putativeSignature) {
	if (!base58Encoder) base58Encoder = getBase58Encoder$1();
	if (putativeSignature.length < 64 || putativeSignature.length > 88) return false;
	return isSignatureBytes(base58Encoder.encode(putativeSignature));
}
...
async function generateKeyPair(extractable = false) {
	...
	return await crypto.subtle.generateKey(ED25519_ALGORITHM_IDENTIFIER, extractable, ["sign", "verify"]);
}
async function createKeyPairFromBytes(bytes, extractable = false) {
	...
	const randomBytes = /* @__PURE__ */ new Uint8Array(32);
...
async function createKeyPairFromPrivateKeyBytes(bytes, extractable = false) {
	...
	const [publicKey, privateKey] = await Promise.all([(extractable ? privateKeyPromise : createPrivateKeyFromBytes(bytes, true)).then(async (privateKey2) => await getPublicKeyFromPrivateKey(privateKey2, true)), privateKeyPromise]);
	return {
		privateKey,
		publicKey
	};
...
function assertGrindRegexIsValid(regex) {
	...
	const isBase58Character = (character) => {
		if (regex.ignoreCase) return BASE58_ALPHABET_REGEX.test(character.toLowerCase()) || BASE58_ALPHABET_REGEX.test(character.toUpperCase());
		return BASE58_ALPHABET_REGEX.test(character);
	};
	for (const character of stripped) if (!isBase58Character(character)) throw new SolanaError(SOLANA_ERROR__KEYS__INVALID_BASE58_IN_GRIND_REGEX, {
		character,
		source: regex.source
	});
}
async function grindKeyPairs(config) {
	...
	let matcher;
	if (typeof matches === "function") matcher = matches;
	else {
		assertGrindRegexIsValid(matches);
		matcher = (address) => matches.test(address);
	}
	if (amount <= 0) return [];
	const base58Decoder = getBase58Decoder$1();
	const found = [];
	while (found.length < amount) {
		abortSignal?.throwIfAborted();
		const batch = await getAbortablePromise(Promise.all(Array.from({ length: concurrency }, async () => {
			const keyPair = await generateKeyPair(extractable);
			const publicKeyBytes = new Uint8Array(await crypto.subtle.exportKey("raw", keyPair.publicKey));
			return {
				address: base58Decoder.decode(publicKeyBytes),
				keyPair
			};
		})), abortSignal);
		for (const { address, keyPair } of batch) {
			if (found.length >= amount) break;
			if (matcher(address)) found.push(keyPair);
		}
	}
	return found;
}
async function grindKeyPair(config) {
	const [keyPair] = await grindKeyPairs({
		...config,
		amount: 1
	});
	return keyPair;
...
//#endregion
//#region ../node_modules/@solana/transactions/dist/index.browser.mjs
function getSignaturesToEncode$1(signaturesMap) {
	...
	return signatures.map((signature) => {
		if (!signature) return (/* @__PURE__ */ new Uint8Array(64)).fill(0);
		return signature;
	});
}
function getSignaturesEncoderWithSizePrefix() {
	return transformEncoder(getArrayEncoder(fixEncoderSize(getBytesEncoder(), 64), { size: getShortU16Encoder() }), getSignaturesToEncode$1);
}
...
function getEnvelopeShapeFromTransactionBytes(transactionBytes) {
	if (transactionBytes.length === 0) throw new SolanaError(SOLANA_ERROR__TRANSACTION__CANNOT_DECODE_EMPTY_TRANSACTION_BYTES);
	const firstByte = transactionBytes[0];
	if ((firstByte & SIGNATURE_COUNT_FLAG_MASK) === 0) return "signaturesFirst";
	const version = firstByte & VERSION_FLAG_MASK;
	if (version === 0) throw new SolanaError(SOLANA_ERROR__TRANSACTION__VERSION_ZERO_MUST_BE_ENCODED_WITH_SIGNATURES_FIRST, {
		firstByte,
		transactionBytes
	});
	if (version === 1) return "messageFirst";
	throw new SolanaError(SOLANA_ERROR__TRANSACTION__VERSION_NUMBER_NOT_SUPPORTED, { unsupportedVersion: version });
}
...
function getTransactionDecoder() {
	return getPredicateDecoder((transactionBytes) => getEnvelopeShapeFromTransactionBytes(transactionBytes) === "signaturesFirst", getTransactionDecoderWithSignaturesFirst(), getTransactionDecoderWithMessageFirst());
}
function getTransactionDecoderWithSignaturesFirst() {
	return transformDecoder(getStructDecoder([["signatures", getArrayDecoder(fixDecoderSize(getBytesDecoder(), 64), { size: getShortU16Decoder() })], ["messageBytes", getBytesDecoder()]]), decodePartiallyDecodedLegacyOrV0Transaction);
}
function getTransactionDecoderWithMessageFirst() {
	return transformDecoder(getPartiallyDecodedTransactionDecoderWithMessageFirst(), decodePartiallyDecodedV1Transaction);
}
function getPartiallyDecodedTransactionDecoderWithMessageFirst() {
	return createDecoder({ read(bytes, offset) {
		...
		if (messageBytesLength < 0) throw new SolanaError(SOLANA_ERROR__TRANSACTION__SIGNATURE_COUNT_TOO_HIGH_FOR_TRANSACTION_BYTES, {
		...
		const [signatures, finalOffset] = getArrayDecoder(fixDecoderSize(getBytesDecoder(), 64), { size: signatureCount }).read(bytes, offset + messageBytesLength);
		return [{
			messageBytes,
			signatures
		}, finalOffset];
...
function decodePartiallyDecodedLegacyOrV0Transaction(transaction) {
	...
	const [_txVersion, numRequiredSignatures, staticAddresses] = getTupleDecoder([
		getTransactionVersionDecoder(),
		padRightDecoder(getU8Decoder(), 2),
		getArrayDecoder(getAddressDecoder(), { size: getShortU16Decoder() })
	]).decode(messageBytes);
	const signerAddresses = staticAddresses.slice(0, numRequiredSignatures);
	if (signerAddresses.length !== signatures.length) throw new SolanaError(SOLANA_ERROR__TRANSACTION__MESSAGE_SIGNATURES_MISMATCH, {
...
function decodePartiallyDecodedV1Transaction(transaction) {
	...
	const signerAddresses = getArrayDecoder(getAddressDecoder(), { size: numRequiredSignatures }).decode(messageBytes, 42);
	if (signerAddresses.length !== signatures.length) throw new SolanaError(SOLANA_ERROR__TRANSACTION__MESSAGE_SIGNATURES_MISMATCH, {
		numRequiredSignatures,
		signaturesLength: signatures.length,
		signerAddresses
	...
	return {
		messageBytes,
		signatures: makeSignaturesMap(signerAddresses, signatures)
	};
}
...
function compiledV1InstructionIsAdvanceNonceInstruction(instructionHeader, instructionPayload, staticAddresses) {
	return staticAddresses[instructionHeader.programAccountIndex] === SYSTEM_PROGRAM_ADDRESS && isAdvanceNonceAccountInstructionData(instructionPayload.instructionData) && instructionHeader.numInstructionAccounts === 3;
}
function compiledLegacyInstructionIsAdvanceNonceInstruction(instruction, staticAddresses) {
	return staticAddresses[instruction.programAddressIndex] === SYSTEM_PROGRAM_ADDRESS && instruction.data != null && isAdvanceNonceAccountInstructionData(instruction.data) && instruction.accountIndices?.length === 3;
}
...
async function getTransactionLifetimeConstraintFromCompiledTransactionMessage(compiledTransactionMessage) {
	...
	if (version === "legacy" || version === 0) {
		...
		if (firstInstruction && compiledLegacyInstructionIsAdvanceNonceInstruction(firstInstruction, staticAccounts)) {
			...
			if (!nonceAccountAddress) throw new SolanaError(SOLANA_ERROR__TRANSACTION__NONCE_ACCOUNT_CANNOT_BE_IN_LOOKUP_TABLE, { nonce: compiledTransactionMessage.lifetimeToken });
			return {
	...
	if (version === 1) {
		...
		const { staticAccounts } = compiledTransactionMessage;
		if (firstInstructionHeader && firstInstructionPayload && compiledV1InstructionIsAdvanceNonceInstruction(firstInstructionHeader, firstInstructionPayload, staticAccounts)) {
			const nonceAccountAddress = staticAccounts[firstInstructionPayload.instructionAccountIndices[0]];
			if (!nonceAccountAddress) throw new SolanaError(SOLANA_ERROR__TRANSACTION__INVALID_NONCE_ACCOUNT_INDEX, {
				nonce: compiledTransactionMessage.lifetimeToken,
...
function assertIsTransactionWithBlockhashLifetime(transaction) {
	if (!isTransactionWithBlockhashLifetime(transaction)) throw new SolanaError(SOLANA_ERROR__TRANSACTION__EXPECTED_BLOCKHASH_LIFETIME);
}
...
function assertIsTransactionWithDurableNonceLifetime(transaction) {
	if (!isTransactionWithDurableNonceLifetime(transaction)) throw new SolanaError(SOLANA_ERROR__TRANSACTION__EXPECTED_NONCE_LIFETIME);
}
...
function getSignatureFromTransaction(transaction) {
	if (!base58Decoder) base58Decoder = getBase58Decoder$1();
	const signatureBytes = Object.values(transaction.signatures)[0];
	if (!signatureBytes) throw new SolanaError(SOLANA_ERROR__TRANSACTION__FEE_PAYER_SIGNATURE_MISSING);
...
function assertIsFullySignedTransaction(transaction) {
	...
	if (missingSigs.length > 0) throw new SolanaError(SOLANA_ERROR__TRANSACTION__SIGNATURES_MISSING, { addresses: missingSigs });
}
...
var TRANSACTION_PACKET_HEADER = 48;
var TRANSACTION_SIZE_LIMIT = TRANSACTION_PACKET_SIZE - 48;
function getTransactionSize(transaction) {
	return getTransactionEncoder().getSizeFromValue(transaction);
}
function getTransactionSizeLimit(transaction) {
	return (transaction.messageBytes[0] & 127) === 1 ? V1_TRANSACTION_SIZE_LIMIT : LEGACY_TRANSACTION_SIZE_LIMIT;
}
function isTransactionWithinSizeLimit(transaction) {
	if (transaction.messageBytes.length === 0) return true;
...
function assertIsTransactionWithinSizeLimit(transaction) {
	if (transaction.messageBytes.length === 0) return;
	const sizeLimit = getTransactionSizeLimit(transaction);
	const transactionSize = getTransactionSize(transaction);
	if (transactionSize > sizeLimit) throw new SolanaError(SOLANA_ERROR__TRANSACTION__EXCEEDS_SIZE_LIMIT, {
		transactionSize,
...
function assertIsTransactionMessageWithinSizeLimit(transactionMessage) {
	...
	if (transactionSize > transactionSizeLimit) throw new SolanaError(SOLANA_ERROR__TRANSACTION__EXCEEDS_SIZE_LIMIT, {
		transactionSize,
		transactionSizeLimit
	});
}
//#endregion
//#region ../node_modules/@solana/instruction-plans/dist/index.browser.mjs
...
function assertValidMaxInstructionsPerTransaction(maxInstructions) {
	if (maxInstructions !== void 0 && (maxInstructions <= 0 || !Number.isInteger(maxInstructions) || maxInstructions > TRANSACTION_INSTRUCTION_LIMIT)) throw new SolanaError(SOLANA_ERROR__INSTRUCTION_PLANS__INVALID_MAX_INSTRUCTIONS_PER_TRANSACTION, {
		maxInstructions,
...
function assertMaxInstructionsPerTransaction(numInstructions, maxInstructions) {
	if (numInstructions > maxInstructions) throw new SolanaError(SOLANA_ERROR__INSTRUCTION_PLANS__MAX_INSTRUCTIONS_PER_TRANSACTION_EXCEEDED, {
		maxInstructions,
		numInstructions
	});
}
...
function assertIsSingleInstructionPlan(plan) {
	if (!isSingleInstructionPlan(plan)) throw new SolanaError(SOLANA_ERROR__INSTRUCTION_PLANS__UNEXPECTED_INSTRUCTION_PLAN, {
...
function assertIsMessagePackerInstructionPlan(plan) {
	if (!isMessagePackerInstructionPlan(plan)) throw new SolanaError(SOLANA_ERROR__INSTRUCTION_PLANS__UNEXPECTED_INSTRUCTION_PLAN, {
		actualKind: plan.kind,
...
function assertIsSequentialInstructionPlan(plan) {
	if (!isSequentialInstructionPlan(plan)) throw new SolanaError(SOLANA_ERROR__INSTRUCTION_PLANS__UNEXPECTED_INSTRUCTION_PLAN, {
		actualKind: plan.kind,
...
function assertIsNonDivisibleSequentialInstructionPlan(plan) {
	if (!isNonDivisibleSequentialInstructionPlan(plan)) throw new SolanaError(SOLANA_ERROR__INSTRUCTION_PLANS__UNEXPECTED_INSTRUCTION_PLAN, {
...
function assertIsParallelInstructionPlan(plan) {
	if (!isParallelInstructionPlan(plan)) throw new SolanaError(SOLANA_ERROR__INSTRUCTION_PLANS__UNEXPECTED_INSTRUCTION_PLAN, {
		actualKind: plan.kind,
...
function findInstructionPlan(instructionPlan, predicate) {
	if (predicate(instructionPlan)) return instructionPlan;
	if (instructionPlan.kind === "single" || instructionPlan.kind === "messagePacker") return;
...
function everyInstructionPlan(instructionPlan, predicate) {
	if (!predicate(instructionPlan)) return false;
	if (instructionPlan.kind === "single" || instructionPlan.kind === "messagePacker") return true;
...
function transformInstructionPlan(instructionPlan, fn) {
	if (instructionPlan.kind === "single" || instructionPlan.kind === "messagePacker") return Object.freeze(fn(instructionPlan));
...
function flattenInstructionPlan(instructionPlan) {
	if (instructionPlan.kind === "single" || instructionPlan.kind === "messagePacker") return [instructionPlan];
		...
		getMessagePacker: () => {
			...
			return Object.freeze({
				...
				packMessageToCapacity: (message, config) => {
					if (offset >= totalBytes) throw new SolanaError(SOLANA_ERROR__INSTRUCTION_PLANS__MESSAGE_PACKER_ALREADY_COMPLETE);
	...
	return Object.freeze({
		getMessagePacker: () => {
			...
			return Object.freeze({
				...
				packMessageToCapacity: (message, config) => {
					if (instructionIndex >= instructions.length) throw new SolanaError(SOLANA_ERROR__INSTRUCTION_PLANS__MESSAGE_PACKER_ALREADY_COMPLETE);
					...
					for (let index = instructionIndex; index < instructions.length; index++) {
						...
						if (messageSize > getTransactionMessageSizeLimit(nextMessage)) {
							if (index === instructionIndex) throw new SolanaError(SOLANA_ERROR__INSTRUCTION_PLANS__MESSAGE_CANNOT_ACCOMMODATE_PLAN, {
...
function getReallocMessagePackerInstructionPlan({ getInstruction, totalSize }) {
	...
	return getMessagePackerInstructionPlanFromInstructions(new Array(numberOfInstructions).fill(0).map((_, i) => getInstruction(i === numberOfInstructions - 1 ? lastInstructionSize : REALLOC_LIMIT)));
}
function appendTransactionMessageInstructionPlan(instructionPlan, transactionMessage) {
	return flattenInstructionPlan(instructionPlan).reduce((messageSoFar, plan) => {
		const kind = plan.kind;
		if (kind === "single") return appendTransactionMessageInstruction(plan.instruction, messageSoFar);
		if (kind === "messagePacker") {
			const messagerPacker = plan.getMessagePacker();
			let nextMessage = messageSoFar;
			while (!messagerPacker.done()) nextMessage = messagerPacker.packMessageToCapacity(nextMessage);
			return nextMessage;
		}
...
function assertIsSingleTransactionPlan(plan) {
	if (!isSingleTransactionPlan(plan)) throw new SolanaError(SOLANA_ERROR__INSTRUCTION_PLANS__UNEXPECTED_TRANSACTION_PLAN, {
		actualKind: plan.kind,
		expectedKind: "single",
...
function assertIsSequentialTransactionPlan(plan) {
	if (!isSequentialTransactionPlan(plan)) throw new SolanaError(SOLANA_ERROR__INSTRUCTION_PLANS__UNEXPECTED_TRANSACTION_PLAN, {
		actualKind: plan.kind,
		expectedKind: "sequential",
		transactionPlan: plan
	});
...
function assertIsNonDivisibleSequentialTransactionPlan(plan) {
	if (!isNonDivisibleSequentialTransactionPlan(plan)) throw new SolanaError(SOLANA_ERROR__INSTRUCTION_PLANS__UNEXPECTED_TRANSACTION_PLAN, {
		actualKind: plan.kind === "sequential" ? "divisible sequential" : plan.kind,
		expectedKind: "non-divisible sequential",
		transactionPlan: plan
	});
...
function assertIsParallelTransactionPlan(plan) {
	if (!isParallelTransactionPlan(plan)) throw new SolanaError(SOLANA_ERROR__INSTRUCTION_PLANS__UNEXPECTED_TRANSACTION_PLAN, {
		actualKind: plan.kind,
		expectedKind: "parallel",
		transactionPlan: plan
	});
...
function flattenTransactionPlan(transactionPlan) {
	if (transactionPlan.kind === "single") return [transactionPlan];
	return transactionPlan.plans.flatMap(flattenTransactionPlan);
}
function findTransactionPlan(transactionPlan, predicate) {
	if (predicate(transactionPlan)) return transactionPlan;
	if (transactionPlan.kind === "single") return;
...
function everyTransactionPlan(transactionPlan, predicate) {
	if (!predicate(transactionPlan)) return false;
	if (transactionPlan.kind === "single") return true;
...
function transformTransactionPlan(transactionPlan, fn) {
	if (transactionPlan.kind === "single") return Object.freeze(fn(transactionPlan));
	return Object.freeze(fn(Object.freeze({
		...transactionPlan,
...
function parseInstructionPlanInput(input) {
	if (Array.isArray(input) && input.length === 1) return parseInstructionPlanInput(input[0]);
	if (Array.isArray(input)) return sequentialInstructionPlan(input.map(parseInstructionPlanInput));
	return isInstructionPlan(input) ? input : singleInstructionPlan(input);
}
function parseTransactionPlanInput(input) {
	if (Array.isArray(input) && input.length === 1) return parseTransactionPlanInput(input[0]);
	if (Array.isArray(input)) return sequentialTransactionPlan(input.map((item) => parseTransactionPlanInput(item)));
	return isTransactionPlan(input) ? input : singleTransactionPlan(input);
}
function parseInstructionOrTransactionPlanInput(input) {
	if (Array.isArray(input) && input.length === 0) return parseTransactionPlanInput(input);
	if (Array.isArray(input) && isTransactionPlanInput(input[0])) return parseTransactionPlanInput(input);
	if (isTransactionPlanInput(input)) return parseTransactionPlanInput(input);
...
function assertIsSuccessfulSingleTransactionPlanResult(plan) {
	if (!isSuccessfulSingleTransactionPlanResult(plan)) throw new SolanaError(SOLANA_ERROR__INSTRUCTION_PLANS__UNEXPECTED_TRANSACTION_PLAN_RESULT, {
...
function assertIsFailedSingleTransactionPlanResult(plan) {
	if (!isFailedSingleTransactionPlanResult(plan)) throw new SolanaError(SOLANA_ERROR__INSTRUCTION_PLANS__UNEXPECTED_TRANSACTION_PLAN_RESULT, {
		actualKind: plan.kind === "single" ? `${plan.status} single` : plan.kind,
...
function assertIsCanceledSingleTransactionPlanResult(plan) {
	if (!isCanceledSingleTransactionPlanResult(plan)) throw new SolanaError(SOLANA_ERROR__INSTRUCTION_PLANS__UNEXPECTED_TRANSACTION_PLAN_RESULT, {
		actualKind: plan.kind === "single" ? `${plan.status} single` : plan.kind,
...
function assertIsSequentialTransactionPlanResult(plan) {
	if (!isSequentialTransactionPlanResult(plan)) throw new SolanaError(SOLANA_ERROR__INSTRUCTION_PLANS__UNEXPECTED_TRANSACTION_PLAN_RESULT, {
		actualKind: plan.kind,
		expectedKind: "sequential",
		transactionPlanResult: plan
	});
...
function assertIsNonDivisibleSequentialTransactionPlanResult(plan) {
	if (!isNonDivisibleSequentialTransactionPlanResult(plan)) throw new SolanaError(SOLANA_ERROR__INSTRUCTION_PLANS__UNEXPECTED_TRANSACTION_PLAN_RESULT, {
		actualKind: plan.kind === "sequential" ? "divisible sequential" : plan.kind,
		expectedKind: "non-divisible sequential",
		transactionPlanResult: plan
	});
...
function assertIsParallelTransactionPlanResult(plan) {
	if (!isParallelTransactionPlanResult(plan)) throw new SolanaError(SOLANA_ERROR__INSTRUCTION_PLANS__UNEXPECTED_TRANSACTION_PLAN_RESULT, {
		actualKind: plan.kind,
		expectedKind: "parallel",
		transactionPlanResult: plan
	});
...
function assertIsSuccessfulTransactionPlanResult(plan) {
	if (!isSuccessfulTransactionPlanResult(plan)) throw new SolanaError(SOLANA_ERROR__INSTRUCTION_PLANS__EXPECTED_SUCCESSFUL_TRANSACTION_PLAN_RESULT, { transactionPlanResult: plan });
}
function findTransactionPlanResult(transactionPlanResult, predicate) {
	if (predicate(transactionPlanResult)) return transactionPlanResult;
	if (transactionPlanResult.kind === "single") return;
...
function getFirstFailedSingleTransactionPlanResult(transactionPlanResult) {
	...
	if (!result) {
		...
		throw new SolanaError(SOLANA_ERROR__INSTRUCTION_PLANS__FAILED_SINGLE_TRANSACTION_PLAN_RESULT_NOT_FOUND, context);
	}
	return result;
}
function everyTransactionPlanResult(transactionPlanResult, predicate) {
	if (!predicate(transactionPlanResult)) return false;
	if (transactionPlanResult.kind === "single") return true;
...
function transformTransactionPlanResult(transactionPlanResult, fn) {
	if (transactionPlanResult.kind === "single") return Object.freeze(fn(transactionPlanResult));
	return Object.freeze(fn(Object.freeze({
...
function summarizeTransactionPlanResult(result) {
	...
	for (const singleResult of flattenedResults) switch (singleResult.status) {
		case "successful":
...
function createFailedToSendTransactionError(result, abortReason) {
	...
	if (result.status === "failed") {
		...
		causeMessage = `${getFailedIndicator(!!preflightData, result.context.signature)}: ${cause.message}${formatLogSnippet(logs)}`;
	} else {
		cause = abortReason;
	...
	return new SolanaError(11, context);
}
...
[Tx #${index + 1}${indicator}] ${error.message}`;
	...
	return new SolanaError(12, context);
}
...
function unwrapErrorWithPreflightData(error) {
	const simulationCodes = [SOLANA_ERROR__JSON_RPC__SERVER_ERROR_SEND_TRANSACTION_PREFLIGHT_FAILURE, SOLANA_ERROR__TRANSACTION__FAILED_WHEN_SIMULATING_TO_ESTIMATE_COMPUTE_LIMIT];
	...
	return {
		logs: void 0,
		preflightData: void 0,
		unwrappedError: error
	};
}
function findErrorFromTransactionPlanResult(result) {
	if (result.kind === "single") return result.status === "failed" ? result.error : void 0;
...
function formatLogSnippet(logs) {
	...
	const maxLines = 8;
	const lastLines = logs.slice(-8);
	return `${logs.length > maxLines ? `
...
function createTransactionPlanExecutor(config) {
	return async (plan, { abortSignal } = {}) => {
		...
		if (traverseConfig.canceled) throw createFailedToExecuteTransactionPlanError(transactionPlanResult, abortSignal?.aborted ? abortSignal.reason : void 0);
...
async function traverse(transactionPlan, traverseConfig) {
	...
	switch (kind) {
		case "sequential": return await traverseSequential(transactionPlan, traverseConfig);
		case "parallel": return await traverseParallel(transactionPlan, traverseConfig);
		case "single": return await traverseSingle(transactionPlan, traverseConfig);
		default: throw new SolanaError(SOLANA_ERROR__INVARIANT_VIOLATION__INVALID_TRANSACTION_PLAN_KIND, { kind });
	}
}
async function traverseSequential(transactionPlan, traverseConfig) {
	if (!transactionPlan.divisible) throw new SolanaError(SOLANA_ERROR__INSTRUCTION_PLANS__NON_DIVISIBLE_TRANSACTION_PLANS_NOT_SUPPORTED);
	const results = [];
	for (const subPlan of transactionPlan.plans) {
...
async function traverseParallel(transactionPlan, traverseConfig) {
	return parallelTransactionPlanResult(await Promise.all(transactionPlan.plans.map((plan) => traverse(plan, traverseConfig))));
}
async function traverseSingle(transactionPlan, traverseConfig) {
	const context = {};
	if (traverseConfig.canceled) return canceledSingleTransactionPlanResult(transactionPlan.message, context);
	try {
		const result = await getAbortablePromise(traverseConfig.executeTransactionMessage(context, transactionPlan.message, { abortSignal: traverseConfig.abortSignal }), traverseConfig.abortSignal);
		return typeof result === "string" ? successfulSingleTransactionPlanResult(transactionPlan.message, {
			...context,
			signature: result
		}) : successfulSingleTransactionPlanResultFromTransaction(transactionPlan.message, result, context);
	} catch (error) {
		traverseConfig.canceled = true;
		const contextWithSignature = "transaction" in context && typeof context.transaction === "object" && context.signature == null ? {
			...context,
...
function assertDivisibleSequentialPlansOnly(transactionPlan) {
	switch (transactionPlan.kind) {
		case "sequential":
			if (!transactionPlan.divisible) throw new SolanaError(SOLANA_ERROR__INSTRUCTION_PLANS__NON_DIVISIBLE_TRANSACTION_PLANS_NOT_SUPPORTED);
			for (const subPlan of transactionPlan.plans) assertDivisibleSequentialPlansOnly(subPlan);
			return;
		case "parallel":
			for (const subPlan of transactionPlan.plans) assertDivisibleSequentialPlansOnly(subPlan);
			return;
...
async function passthroughFailedTransactionPlanExecution(promise) {
	try {
		...
		if (isSolanaError(error, 7618003)) return error.context.transactionPlanResult;
		throw error;
	}
}
function createTransactionPlanner(config) {
	return async (instructionPlan, { abortSignal, maxInstructionsPerTransaction } = {}) => {
		...
		if (!plan) throw new SolanaError(SOLANA_ERROR__INSTRUCTION_PLANS__EMPTY_INSTRUCTION_PLAN);
...
async function traverse2(instructionPlan, context) {
	...
	switch (kind) {
		case "sequential": return await traverseSequential2(instructionPlan, context);
		case "parallel": return await traverseParallel2(instructionPlan, context);
		case "single": return await traverseSingle2(instructionPlan, context);
		case "messagePacker": return await traverseMessagePacker(instructionPlan, context);
		default: throw new SolanaError(SOLANA_ERROR__INVARIANT_VIOLATION__INVALID_INSTRUCTION_PLAN_KIND, { kind });
	}
}
async function traverseSequential2(instructionPlan, context) {
	let candidate = null;
	if (context.parent && (context.parent.kind === "parallel" || !instructionPlan.divisible)) {
		if (await selectAndMutateCandidate(context, context.parentCandidates, (message) => fitEntirePlanInsideMessage(context, instructionPlan, message))) return null;
	} else candidate = context.parentCandidates.length > 0 ? context.parentCandidates[0] : null;
	...
	if (transactionPlans.length === 1) return transactionPlans[0];
	if (transactionPlans.length === 0) return null;
	return {
...
async function traverseSingle2(instructionPlan, context) {
	...
	if (await selectAndMutateCandidate(context, context.parentCandidates, predicate)) return null;
	return {
		kind: "single",
		message: await createNewMessage(context, predicate),
		planType: "transactionPlan"
	};
}
async function traverseMessagePacker(instructionPlan, context) {
	...
	const transactionPlans = [];
	const candidates = [...context.parentCandidates];
	const packMessageToCapacity = (message) => messagePacker.packMessageToCapacity(message, { maxInstructions: context.maxInstructionsPerTransaction });
	while (!messagePacker.done()) if (!await selectAndMutateCandidate(context, candidates, packMessageToCapacity)) {
		const newPlan = {
			kind: "single",
			message: await createNewMessage(context, packMessageToCapacity),
			planType: "transactionPlan"
		};
	...
	if (transactionPlans.length === 1) return transactionPlans[0];
	if (transactionPlans.length === 0) return null;
	if (context.parent?.kind === "parallel") return {
		kind: "parallel",
		planType: "transactionPlan",
		plans: transactionPlans
	};
...
function getSequentialCandidate(latestPlan) {
	if (latestPlan.kind === "single") return latestPlan;
	if (latestPlan.kind === "sequential" && latestPlan.plans.length > 0) return getSequentialCandidate(latestPlan.plans[latestPlan.plans.length - 1]);
	return null;
}
...
async function selectAndMutateCandidate(context, candidates, predicate) {
	for (const candidate of candidates) try {
		const message = await getAbortablePromise(Promise.resolve(context.onTransactionMessageUpdated(predicate(candidate.message), { abortSignal: context.abortSignal })), context.abortSignal);
		if (getTransactionMessageSize(message) <= getTransactionMessageSizeLimit(message)) {
			assertMaxInstructionsPerTransaction(message.instructions.length, resolveMaxInstructions(context.maxInstructionsPerTransaction));
			candidate.message = message;
			return candidate;
		}
	} catch (error) {
		if (isSolanaError(error, 7618010) || isSolanaError(error, 7618e3) || isSolanaError(error, 5663033) || isSolanaError(error, 5663035) || isSolanaError(error, 5663034) || isSolanaError(error, 5663032));
		else throw error;
	}
	return null;
}
async function createNewMessage(context, predicate) {
	...
	assertMaxInstructionsPerTransaction(updatedMessage.instructions.length, resolveMaxInstructions(context.maxInstructionsPerTransaction));
	return updatedMessage;
}
function freezeTransactionPlan(plan) {
	const kind = plan.kind;
	switch (kind) {
		case "single": return singleTransactionPlan(plan.message);
		case "sequential": return plan.divisible ? sequentialTransactionPlan(plan.plans.map(freezeTransactionPlan)) : nonDivisibleSequentialTransactionPlan(plan.plans.map(freezeTransactionPlan));
...
function fitEntirePlanInsideMessage(context, instructionPlan, message) {
	...
	switch (kind) {
		case "sequential":
		case "parallel":
			for (const plan of instructionPlan.plans) newMessage = fitEntirePlanInsideMessage(context, plan, newMessage);
			return newMessage;
		case "single":
			...
			assertMaxInstructionsPerTransaction(newMessage.instructions.length, resolveMaxInstructions(context.maxInstructionsPerTransaction));
			return newMessage;
		case "messagePacker":
			const messagePacker = instructionPlan.getMessagePacker();
			while (!messagePacker.done()) {
				newMessage = messagePacker.packMessageToCapacity(newMessage, { maxInstructions: context.maxInstructionsPerTransaction });
				assertMaxInstructionsPerTransaction(newMessage.instructions.length, resolveMaxInstructions(context.maxInstructionsPerTransaction));
			}
			return newMessage;
		default: throw new SolanaError(SOLANA_ERROR__INVARIANT_VIOLATION__INVALID_INSTRUCTION_PLAN_KIND, { kind });
...
//#endregion
//#region ../node_modules/@solana/offchain-messages/dist/index.browser.mjs
...
function assertIsOffchainMessageApplicationDomain(putativeApplicationDomain) {
	try {
		...
		if (isSolanaError(error, 2800001)) throw new SolanaError(SOLANA_ERROR__OFFCHAIN_MESSAGE__APPLICATION_DOMAIN_STRING_LENGTH_OUT_OF_RANGE, error.context);
		if (isSolanaError(error, 28e5)) throw new SolanaError(SOLANA_ERROR__OFFCHAIN_MESSAGE__INVALID_APPLICATION_DOMAIN_BYTE_LENGTH, error.context);
		throw error;
	}
}
...
function getVersionTransformer(fixedVersion) {
	return (version) => {
		if (version > 1) throw new SolanaError(SOLANA_ERROR__OFFCHAIN_MESSAGE__VERSION_NUMBER_NOT_SUPPORTED, { unsupportedVersion: version });
		if (fixedVersion != null && version !== fixedVersion) throw new SolanaError(SOLANA_ERROR__OFFCHAIN_MESSAGE__UNEXPECTED_VERSION, {
...
function decodeRequiredSignatoryAddresses(bytes) {
	...
	return offsetDecoder(transformDecoder(getArrayDecoder(getAddressDecoder(), { size: getU8Decoder() }), (signatoryAddresses) => {
		if (signatoryAddresses.length === 0) throw new SolanaError(SOLANA_ERROR__OFFCHAIN_MESSAGE__NUM_REQUIRED_SIGNERS_CANNOT_BE_ZERO);
		return signatoryAddresses;
	}), { preOffset: ({ preOffset }) => preOffset + (version === 0 ? 33 : 0) }).decode(bytesAfterVersion);
}
function getSignatoriesComparator() {
	return (x, y) => {
		if (x.length !== y.length) return x.length < y.length ? -1 : 1;
		for (let ii = 0; ii < x.length; ii++) if (x[ii] === y[ii]) continue;
		else return x[ii] < y[ii] ? -1 : 1;
		return 0;
	};
}
function getSignaturesToEncode(signaturesMap) {
	const signatures = Object.values(signaturesMap);
	if (signatures.length === 0) throw new SolanaError(SOLANA_ERROR__OFFCHAIN_MESSAGE__NUM_ENVELOPE_SIGNATURES_CANNOT_BE_ZERO);
	return signatures.map((signature) => {
		if (!signature) return (/* @__PURE__ */ new Uint8Array(64)).fill(0);
		return signature;
	});
...
function getSignaturesEncoder() {
	return transformEncoder(getArrayEncoder(fixEncoderSize(getBytesEncoder(), 64), { size: getU8Encoder() }), getSignaturesToEncode);
}
function getOffchainMessageEnvelopeEncoder() {
	return transformEncoder(getStructEncoder([["signatures", getSignaturesEncoder()], ["content", getBytesEncoder()]]), (envelope) => {
		const signaturesMapAddresses = Object.keys(envelope.signatures).map(address);
		if (signaturesMapAddresses.length === 0) throw new SolanaError(SOLANA_ERROR__OFFCHAIN_MESSAGE__NUM_ENVELOPE_SIGNATURES_CANNOT_BE_ZERO);
		...
		for (const address2 of signatoryAddresses) if (!signaturesMapAddresses.includes(address2)) missingRequiredSigners.push(address2);
		for (const address2 of signaturesMapAddresses) if (!signatoryAddresses.includes(address2)) unexpectedSigners.push(address2);
		if (missingRequiredSigners.length || unexpectedSigners.length) throw new SolanaError(SOLANA_ERROR__OFFCHAIN_MESSAGE__ENVELOPE_SIGNERS_MISMATCH, {
			missingRequiredSigners,
			unexpectedSigners
		});
		const orderedSignatureMap = {};
		for (const address2 of signatoryAddresses) orderedSignatureMap[address2] = envelope.signatures[address2];
		return {
			...envelope,
...
function getOffchainMessageEnvelopeDecoder() {
	return transformDecoder(getStructDecoder([["signatures", getArrayDecoder(fixDecoderSize(getBytesDecoder(), 64), { size: getU8Decoder() })], ["content", getBytesDecoder()]]), decodePartiallyDecodedOffchainMessageEnvelope);
}
...
function decodePartiallyDecodedOffchainMessageEnvelope(offchainMessageEnvelope) {
	...
	if (signatoryAddresses.length !== signatures.length) throw new SolanaError(SOLANA_ERROR__OFFCHAIN_MESSAGE__NUM_SIGNATURES_MISMATCH, {
	...
	signatoryAddresses.forEach((address2, index) => {
		const signatureForAddress = signatures[index];
		if (signatureForAddress.every((b) => b === 0)) signaturesMap[address2] = null;
		else signaturesMap[address2] = signatureForAddress;
	});
	return Object.freeze({
		content,
		signatures: Object.freeze(signaturesMap)
...
var MAX_BODY_BYTES = 65535;
var MAX_BODY_BYTES_HARDWARE_WALLET_SIGNABLE = 1232;
...
function assertIsOffchainMessageContentRestrictedAsciiOf1232BytesMax(putativeContent) {
	if (putativeContent.format !== 0) throw new SolanaError(SOLANA_ERROR__OFFCHAIN_MESSAGE__MESSAGE_FORMAT_MISMATCH, {
	...
	if (isTextRestrictedAscii(putativeContent.text) === false) throw new SolanaError(SOLANA_ERROR__OFFCHAIN_MESSAGE__RESTRICTED_ASCII_BODY_CHARACTER_OUT_OF_RANGE);
	const length = getUtf8Encoder().getSizeFromValue(putativeContent.text);
	if (length > MAX_BODY_BYTES_HARDWARE_WALLET_SIGNABLE) throw new SolanaError(SOLANA_ERROR__OFFCHAIN_MESSAGE__MAXIMUM_LENGTH_EXCEEDED, {
		actualBytes: length,
...
function isOffchainMessageContentRestrictedAsciiOf1232BytesMax(putativeContent) {
	if (putativeContent.format !== 0 || putativeContent.text.length === 0 || isTextRestrictedAscii(putativeContent.text) === false) return false;
	return getUtf8Encoder().getSizeFromValue(putativeContent.text) <= MAX_BODY_BYTES_HARDWARE_WALLET_SIGNABLE;
}
...
function isOffchainMessageContentUtf8Of1232BytesMax(putativeContent) {
	if (putativeContent.format !== 1 || putativeContent.text.length === 0) return false;
	return getUtf8Encoder().getSizeFromValue(putativeContent.text) <= MAX_BODY_BYTES_HARDWARE_WALLET_SIGNABLE;
}
...
function assertIsOffchainMessageContentUtf8Of65535BytesMax(putativeContent) {
	if (putativeContent.format !== 2) throw new SolanaError(SOLANA_ERROR__OFFCHAIN_MESSAGE__MESSAGE_FORMAT_MISMATCH, {
		actualMessageFormat: putativeContent.format,
		expectedMessageFormat: 2
	});
	if (putativeContent.text.length === 0) throw new SolanaError(SOLANA_ERROR__OFFCHAIN_MESSAGE__MESSAGE_MUST_BE_NON_EMPTY);
	const length = getUtf8Encoder().getSizeFromValue(putativeContent.text);
	if (length > MAX_BODY_BYTES) throw new SolanaError(SOLANA_ERROR__OFFCHAIN_MESSAGE__MAXIMUM_LENGTH_EXCEEDED, {
		actualBytes: length,
		maxBytes: MAX_BODY_BYTES
	});
}
function isOffchainMessageContentUtf8Of65535BytesMax(putativeContent) {
	if (putativeContent.format !== 2 || putativeContent.text.length === 0) return false;
	return getUtf8Encoder().getSizeFromValue(putativeContent.text) <= MAX_BODY_BYTES;
}
...
function getOffchainMessageContentFormatDecoder() {
	return getEnumDecoder(OffchainMessageContentFormat, { useValuesAsDiscriminators: true });
}
function getOffchainMessageContentFormatEncoder() {
	return getEnumEncoder(OffchainMessageContentFormat, { useValuesAsDiscriminators: true });
}
function getOffchainMessageV0PreambleDecoder() {
	return createOffchainMessagePreambleDecoder(0, ["applicationDomain", getOffchainMessageApplicationDomainDecoder()], ["messageFormat", getOffchainMessageContentFormatDecoder()], ["requiredSignatories", transformDecoder(getArrayDecoder(getAddressDecoder(), { size: getU8Decoder() }), (signatoryAddresses) => {
		if (signatoryAddresses.length === 0) throw new SolanaError(SOLANA_ERROR__OFFCHAIN_MESSAGE__NUM_REQUIRED_SIGNERS_CANNOT_BE_ZERO);
...
function getOffchainMessageV0PreambleEncoder() {
	return createOffchainMessagePreambleEncoder(0, ["applicationDomain", getOffchainMessageApplicationDomainEncoder()], ["messageFormat", getOffchainMessageContentFormatEncoder()], ["requiredSignatories", transformEncoder(getArrayEncoder(getAddressEncoder(), { size: getU8Encoder() }), (signatoryAddresses) => {
		if (signatoryAddresses.length === 0) throw new SolanaError(SOLANA_ERROR__OFFCHAIN_MESSAGE__NUM_REQUIRED_SIGNERS_CANNOT_BE_ZERO);
...
function getOffchainMessageV0Decoder() {
	return transformDecoder(getTupleDecoder([getOffchainMessageV0PreambleDecoder(), getUtf8Decoder()]), ([{ messageLength, messageFormat, requiredSignatories, ...preambleRest }, text]) => {
		...
		if (messageLength !== actualLength) throw new SolanaError(SOLANA_ERROR__OFFCHAIN_MESSAGE__MESSAGE_LENGTH_MISMATCH, {
			actualLength,
		...
		switch (messageFormat) {
			...
			case 2:
				...
				return offchainMessage;
			default: throw new SolanaError(SOLANA_ERROR__INVARIANT_VIOLATION__SWITCH_MUST_BE_EXHAUSTIVE, { unexpectedValue: messageFormat });
		}
	});
}
	...
	return transformEncoder(getTupleEncoder([getOffchainMessageV0PreambleEncoder(), getUtf8Encoder()]), (offchainMessage) => {
		...
		switch (offchainMessage.content.format) {
			...
			case 2:
				assertIsOffchainMessageUtf8Of65535BytesMax(offchainMessage);
				break;
			default: throw new SolanaError(SOLANA_ERROR__INVARIANT_VIOLATION__SWITCH_MUST_BE_EXHAUSTIVE, { unexpectedValue: offchainMessage.content });
		}
...
function getOffchainMessageV1PreambleDecoder() {
	return createOffchainMessagePreambleDecoder(1, ["requiredSignatories", transformDecoder(getArrayDecoder(fixDecoderSize(getBytesDecoder(), 32), { size: getU8Decoder() }), (signatoryAddressesBytes) => {
		if (signatoryAddressesBytes.length === 0) throw new SolanaError(SOLANA_ERROR__OFFCHAIN_MESSAGE__NUM_REQUIRED_SIGNERS_CANNOT_BE_ZERO);
		const comparator = getSignatoriesComparator();
		for (let ii = 0; ii < signatoryAddressesBytes.length - 1; ii++) switch (comparator(signatoryAddressesBytes[ii], signatoryAddressesBytes[ii + 1])) {
			case 0: throw new SolanaError(SOLANA_ERROR__OFFCHAIN_MESSAGE__SIGNATORIES_MUST_BE_UNIQUE);
			case 1: throw new SolanaError(SOLANA_ERROR__OFFCHAIN_MESSAGE__SIGNATORIES_MUST_BE_SORTED);
		}
		const addressDecoder = getAddressDecoder();
		return signatoryAddressesBytes.map((addressBytes) => Object.freeze({ address: addressDecoder.decode(addressBytes) }));
	})]);
}
function getOffchainMessageV1PreambleEncoder() {
	return createOffchainMessagePreambleEncoder(1, ["requiredSignatories", transformEncoder(transformEncoder(getArrayEncoder(getBytesEncoder(), { size: getU8Encoder() }), (signatoryAddressesBytes) => {
		...
		for (const { address: address2 } of signatoryAddresses) {
			if (seenSignatories.has(address2)) throw new SolanaError(SOLANA_ERROR__OFFCHAIN_MESSAGE__SIGNATORIES_MUST_BE_UNIQUE);
...
function getOffchainMessageV1Decoder() {
	return transformDecoder(getTupleDecoder([getOffchainMessageV1PreambleDecoder(), getUtf8Decoder()]), ([{ requiredSignatories, ...preambleRest }, text]) => {
		if (text.length === 0) throw new SolanaError(SOLANA_ERROR__OFFCHAIN_MESSAGE__MESSAGE_MUST_BE_NON_EMPTY);
		return Object.freeze({
			...preambleRest,
			content: text,
			requiredSignatories: Object.freeze(requiredSignatories)
...
function getOffchainMessageV1Encoder() {
	return transformEncoder(getTupleEncoder([getOffchainMessageV1PreambleEncoder(), getUtf8Encoder()]), (offchainMessage) => {
		...
		if (content.length === 0) throw new SolanaError(SOLANA_ERROR__OFFCHAIN_MESSAGE__MESSAGE_MUST_BE_NON_EMPTY);
...
function getOffchainMessageDecoder() {
	return createDecoder({ read(bytes, offset) {
		const version = getHiddenPrefixDecoder(getU8Decoder(), [getOffchainMessageSigningDomainDecoder()]).decode(bytes, offset);
		switch (version) {
			case 0: return getOffchainMessageV0Decoder().read(bytes, offset);
			case 1: return getOffchainMessageV1Decoder().read(bytes, offset);
			default: throw new SolanaError(SOLANA_ERROR__OFFCHAIN_MESSAGE__VERSION_NUMBER_NOT_SUPPORTED, { unsupportedVersion: version });
		}
	} });
}
function getOffchainMessageEncoder() {
	return createEncoder({
		getSizeFromValue: (offchainMessage) => {
			...
			switch (version) {
				case 0: return getOffchainMessageV0Encoder().getSizeFromValue(offchainMessage);
				case 1: return getOffchainMessageV1Encoder().getSizeFromValue(offchainMessage);
				default: throw new SolanaError(SOLANA_ERROR__OFFCHAIN_MESSAGE__VERSION_NUMBER_NOT_SUPPORTED, { unsupportedVersion: version });
			}
		},
		write: (offchainMessage, bytes, offset) => {
			const { version } = offchainMessage;
			switch (version) {
				case 0: return getOffchainMessageV0Encoder().write(offchainMessage, bytes, offset);
				case 1: return getOffchainMessageV1Encoder().write(offchainMessage, bytes, offset);
				default: throw new SolanaError(SOLANA_ERROR__OFFCHAIN_MESSAGE__VERSION_NUMBER_NOT_SUPPORTED, { unsupportedVersion: version });
			}
		}
	});
...
function compileOffchainMessageEnvelopeUsingEncoder(offchainMessage, encoder) {
	...
	const signatures = {};
	for (const { address: address2 } of offchainMessage.requiredSignatories) signatures[address2] = null;
	return Object.freeze({
...
function compileOffchainMessageEnvelope(offchainMessage) {
	const { version } = offchainMessage;
	switch (version) {
		case 0: return compileOffchainMessageV0Envelope(offchainMessage);
		case 1: return compileOffchainMessageV1Envelope(offchainMessage);
		default: throw new SolanaError(SOLANA_ERROR__INVARIANT_VIOLATION__SWITCH_MUST_BE_EXHAUSTIVE, { unexpectedValue: version });
	}
}
async function partiallySignOffchainMessageEnvelope(keyPairs, offchainMessageEnvelope) {
	...
	if (unexpectedSigners && unexpectedSigners.size > 0) throw new SolanaError(SOLANA_ERROR__OFFCHAIN_MESSAGE__ADDRESSES_CANNOT_SIGN_OFFCHAIN_MESSAGE, {
		expectedAddresses: requiredSignatoryAddresses,
		unexpectedAddresses: [...unexpectedSigners]
	});
	if (!newSignatures) return offchainMessageEnvelope;
	return Object.freeze({
...
function assertIsFullySignedOffchainMessageEnvelope(offchainMessage) {
	...
	Object.entries(offchainMessage.signatures).forEach(([address2, signatureBytes]) => {
		if (!signatureBytes) missingSigs.push(address2);
	});
	if (missingSigs.length > 0) throw new SolanaError(SOLANA_ERROR__OFFCHAIN_MESSAGE__SIGNATURES_MISSING, { addresses: missingSigs });
}
async function verifyOffchainMessageEnvelope(offchainMessageEnvelope) {
	...
	await Promise.all(requiredSignatories.map(async (address2) => {
		...
		} else if (await verifySignature(await getPublicKeyFromAddress(address2), signature, offchainMessageEnvelope.content)) return true;
		else {
			errorContext ||= {};
	...
	if (errorContext) throw new SolanaError(SOLANA_ERROR__OFFCHAIN_MESSAGE__SIGNATURE_VERIFICATION_FAILURE, errorContext);
}
//#endregion
//#region ../node_modules/@solana/plugin-core/dist/index.browser.mjs
...
function toConfigurableDescriptors(descriptors) {
	const result = {};
	for (const key of Reflect.ownKeys(descriptors)) result[key] = {
		...descriptors[key],
		configurable: true
	};
	return result;
...
function withCleanup(client, cleanup) {
	if (DISPOSABLE_STACK_PROPERTY in client) return addCleanupToClientWithExistingStack(client, cleanup);
	else return addCleanupToClientWithoutExistingStack(client, cleanup);
}
...
//#endregion
//#region ../node_modules/@solana/programs/dist/index.browser.mjs
function isProgramError(error, transactionMessage, programAddress, code) {
	if (!isSolanaError(error, 4615026)) return false;
...
//#endregion
//#region ../node_modules/@solana/subscribable/dist/index.browser.mjs
var o$5 = globalThis.AbortController;
var t$1 = globalThis.EventTarget;
var IDLE_STATE$1 = Object.freeze({
	data: void 0,
	error: void 0,
	status: "idle"
});
function createReactiveActionStore(fn) {
	let state = IDLE_STATE$1;
	let currentController;
	const listeners = /* @__PURE__ */ new Set();
	function setState(next) {
		if (state.status === next.status && state.data === next.data && state.error === next.error) return;
		state = next;
...
function createAsyncIterableFromDataPublisher({ abortSignal, dataChannelName, dataPublisher, errorChannelName }) {
	...
	function publishErrorToAllIterators(reason) {
		for (const [iteratorKey, state] of iteratorState.entries()) if (state.__hasPolled) {
			iteratorState.delete(iteratorKey);
			state.onError(reason);
		} else state.publishQueue.push({
			__type: 1,
			err: reason
		});
	}
	const abortController = new o$5();
	abortSignal.addEventListener("abort", () => {
		abortController.abort();
		publishErrorToAllIterators(EXPLICIT_ABORT_TOKEN$1 ||= createExplicitAbortToken$1());
	});
	...
	return { async *[Symbol.asyncIterator]() {
		...
		if (firstError !== UNINITIALIZED) throw firstError;
		...
		try {
			while (true) {
				...
				if (!state) throw new SolanaError(SOLANA_ERROR__INVARIANT_VIOLATION__SUBSCRIPTION_ITERATOR_STATE_MISSING);
				if (state.__hasPolled) throw new SolanaError(SOLANA_ERROR__INVARIANT_VIOLATION__SUBSCRIPTION_ITERATOR_MUST_NOT_POLL_BEFORE_RESOLVING_EXISTING_MESSAGE_PROMISE);
				const publishQueue = state.publishQueue;
				try {
					if (publishQueue.length) {
						state.publishQueue = [];
						for (const item of publishQueue) if (item.__type === 0) yield item.data;
						else throw item.err;
					} else yield await new Promise((resolve, reject) => {
						iteratorState.set(iteratorKey, {
							__hasPolled: true,
							onData: resolve,
							onError: reject
					...
					if (e === (EXPLICIT_ABORT_TOKEN$1 ||= createExplicitAbortToken$1())) return;
					else throw e;
				}
			}
		} finally {
...
function getDataPublisherFromEventEmitter(eventEmitter) {
	return { on(channelName, subscriber, options) {
...
function demultiplexDataPublisher(publisher, sourceChannelName, messageTransformer) {
	...
	return {
		...
		on(channelName, subscriber, options) {
			if (!innerPublisherState) innerPublisherState = {
				dispose: publisher.on(sourceChannelName, (sourceMessage) => {
					const transformResult = messageTransformer(sourceMessage);
					if (!transformResult) return;
					const [destinationChannelName, message] = transformResult;
					eventTarget.dispatchEvent(new CustomEvent(destinationChannelName, { detail: message }));
				}),
				numSubscribers: 0
			};
...
function createReactiveStoreFromDataPublisherFactory({ createDataPublisher, dataChannelName, errorChannelName }) {
	...
	function setState(next) {
		if (currentState.status === next.status && currentState.data === next.data && currentState.error === next.error) return;
		currentState = next;
		notify();
	}
	function performConnect(callerSignal) {
		...
		if (callerSignal) callerSignal.addEventListener("abort", () => {
		...
		createDataPublisher(signal).then((publisher) => {
...
//#endregion
//#region ../node_modules/@solana/rpc-spec-types/dist/index.browser.mjs
...
function unwrapBigIntValueObject({ $n }) {
	...
	if (match) {
		...
		if (mantissa.length + (exponent ? Number(exponent) : 0) <= MAX_BIGINT_DIGITS) return exponent ? BigInt(`${sign}${mantissa}`) * 10n ** BigInt(exponent) : BigInt($n);
	}
	throw new SolanaError(7, { value: $n });
}
...
//#endregion
//#region ../node_modules/@solana/rpc-spec/dist/index.browser.mjs
...
function createPendingRpcRequest({ transport }, plan) {
	return {
		reactiveStore() {
			return createReactiveActionStore((signal) => plan.execute({
				signal,
				transport
			}));
		},
...
//#endregion
//#region ../node_modules/@solana/rpc-transformers/dist/index.browser.mjs
...
function getDefaultRequestTransformerForSolanaRpc(config) {
	...
	return (request) => {
		return pipe(request, handleIntegerOverflow ? getIntegerOverflowRequestTransformer(handleIntegerOverflow) : (r) => r, getDefaultCommitmentRequestTransformer({
...
//#endregion
//#region ../node_modules/@solana/rpc-api/dist/index.browser.mjs
function createSolanaRpcApi(config) {
	return createJsonRpcApi({
		requestTransformer: getDefaultRequestTransformerForSolanaRpc(config),
		responseTransformer: getDefaultResponseTransformerForSolanaRpc({ allowedNumericKeyPaths: getAllowedNumericKeypaths$1() })
	});
}
var memoizedKeypaths$1;
function getAllowedNumericKeypaths$1() {
	if (!memoizedKeypaths$1) memoizedKeypaths$1 = {
...
//#endregion
//#region ../node_modules/@solana/rpc-transport-http/dist/index.browser.mjs
...
var FORBIDDEN_HEADERS = /* @__PURE__ */ Object.assign({
	...
	upgrade: true,
	via: true
}, { "accept-encoding": true }, { origin: true });
...
function warnDispatcherWasSuppliedInNonNodeEnvironment() {
	if (didWarnDispatcherWasSuppliedInNonNodeEnvironment) return;
...
function createHttpTransport(config) {
	if ("dispatcher_NODE_ONLY" in config) warnDispatcherWasSuppliedInNonNodeEnvironment();
	const { fromJson, headers, toJson, url } = config;
	if (headers) assertIsAllowedHttpRequestHeaders(headers);
	const customHeaders = headers && normalizeHeaders$1(headers);
	return async function makeHttpRequest({ payload, signal }) {
		const requestInfo = {
			body: toJson ? toJson(payload) : JSON.stringify(payload),
			headers: {
				...customHeaders,
...
//#endregion
//#region ../node_modules/@solana/fast-stable-stringify/dist/index.browser.mjs
...
//#endregion
//#region ../node_modules/@solana/rpc/dist/index.browser.mjs
...
var o$4 = globalThis.AbortController;
var EXPLICIT_ABORT_TOKEN;
function createExplicitAbortToken() {
	return { EXPLICIT_ABORT_TOKEN: "This object is thrown from the request that underlies a series of coalesced requests when the last request in that series aborts" };
}
		...
		if (coalescedRequestsByDeduplicationKey[deduplicationKey] == null) {
			...
			const responsePromise = (async () => {
				try {
					...
					if (e === (EXPLICIT_ABORT_TOKEN ||= createExplicitAbortToken())) return;
					throw e;
				}
			})();
...
//#endregion
//#region ../node_modules/@solana/rpc-subscriptions-spec/dist/index.browser.mjs
	...
	return new Proxy(rpcConfig.api, {
		...
		get(target, p, receiver) {
			...
			return function(...rawParams) {
				...
				if (!createRpcSubscriptionPlan) throw new SolanaError(SOLANA_ERROR__RPC_SUBSCRIPTIONS__CANNOT_CREATE_SUBSCRIPTION_PLAN, { notificationName });
...
function transformChannelInboundMessages(channel, transform) {
	return Object.freeze({
		...
		on(type, subscriber, options) {
			if (type !== "message") return channel.on(type, subscriber, options);
...
function getSubscriberCountBySubscriptionIdForChannel(channel) {
	...
	if (!subscriberCountBySubscriptionId) subscriberCountBySubscriptionIdByChannel.set(channel, subscriberCountBySubscriptionId = {});
	return subscriberCountBySubscriptionId;
}
function augmentSubscriberCountAndReturnNewCount(amount, channel, subscriptionId) {
	if (subscriptionId === void 0) return;
	const subscriberCountBySubscriptionId = getSubscriberCountBySubscriptionIdForChannel(channel);
	if (!subscriberCountBySubscriptionId[subscriptionId] && amount > 0) subscriberCountBySubscriptionId[subscriptionId] = 0;
	const newCount = amount + subscriberCountBySubscriptionId[subscriptionId];
	if (newCount <= 0) delete subscriberCountBySubscriptionId[subscriptionId];
	else subscriberCountBySubscriptionId[subscriptionId] = newCount;
	return newCount;
}
...
function getMemoizedDemultiplexedNotificationPublisherFromChannelAndResponseTransformer(channel, subscribeRequest, responseTransformer) {
	...
	if (!publisherByResponseTransformer) cache.set(channel, publisherByResponseTransformer = /* @__PURE__ */ new WeakMap());
	...
	if (!publisher) publisherByResponseTransformer.set(responseTransformerKey, publisher = demultiplexDataPublisher(channel, "message", (rawMessage) => {
...
async function executeRpcPubSubSubscriptionPlan({ channel, responseTransformer, signal, subscribeRequest, unsubscribeMethodName }) {
	...
	const abortPromise = new Promise((_, reject) => {
		...
		if (signal.aborted) handleAbort.call(signal);
	...
	subscriptionId = await safeRace([abortPromise, new Promise((resolve, reject) => {
		...
		channel.on("message", (message) => {
			if (message && typeof message === "object" && "id" in message && message.id === subscribePayload.id) {
				abortController.abort();
				if ("error" in message) reject(getSolanaErrorFromJsonRpcError(message.error));
				else resolve(message.result);
			}
		}, options);
	})]);
	if (subscriptionId == null) throw new SolanaError(SOLANA_ERROR__RPC_SUBSCRIPTIONS__EXPECTED_SERVER_SUBSCRIPTION_ID);
	...
	return { on(type, listener, options) {
		switch (type) {
			case "notification": return notificationPublisher.on(notificationKey, listener, options);
			case "error": return channel.on("error", listener, options);
			default: throw new SolanaError(SOLANA_ERROR__INVARIANT_VIOLATION__DATA_PUBLISHER_CHANNEL_UNIMPLEMENTED, {
				channelName: type,
...
//#endregion
//#region ../node_modules/@solana/rpc-subscriptions-api/dist/index.browser.mjs
function createSolanaRpcSubscriptionsApi_INTERNAL(config) {
	...
	const responseTransformer = getDefaultResponseTransformerForSolanaRpcSubscriptions({ allowedNumericKeyPaths: getAllowedNumericKeypaths() });
...
//#endregion
//#region ../node_modules/@solana/rpc-subscriptions-channel-websocket/dist/index.browser.mjs
...
function createWebSocketChannel({ sendBufferHighWatermark, signal, url }) {
	...
	function handleAbort() {
		cleanupListeners();
		if (!hasConnected) rejectOpen(signal.reason);
		if (webSocket.readyState !== e.CLOSED && webSocket.readyState !== e.CLOSING) webSocket.close(NORMAL_CLOSURE_CODE);
	}
	function handleClose(ev) {
		...
		if (!signal.aborted && !(ev.wasClean && ev.code === NORMAL_CLOSURE_CODE)) eventTarget.dispatchEvent(new CustomEvent("error", { detail: new SolanaError(SOLANA_ERROR__RPC_SUBSCRIPTIONS__CHANNEL_CONNECTION_CLOSED, { cause: ev }) }));
	}
	function handleError(ev) {
		if (signal.aborted) return;
		if (!hasConnected) {
			const failedToConnectError = new SolanaError(SOLANA_ERROR__RPC_SUBSCRIPTIONS__CHANNEL_FAILED_TO_CONNECT, { errorEvent: ev });
			rejectOpen(failedToConnectError);
			eventTarget.dispatchEvent(new CustomEvent("error", { detail: failedToConnectError }));
		}
	}
	...
	function handleOpen() {
		...
		resolveOpen({
			...dataPublisher,
			async send(message) {
				if (webSocket.readyState !== e.OPEN) throw new SolanaError(SOLANA_ERROR__RPC_SUBSCRIPTIONS__CHANNEL_CONNECTION_CLOSED);
...
//#endregion
//#region ../node_modules/@solana/rpc-subscriptions/dist/index.browser.mjs
...
function getRpcSubscriptionsChannelWithAutoping({ abortSignal: callerAbortSignal, channel, intervalMs }) {
	...
	function sendPing() {
		channel.send(PING_PAYLOAD).catch((e) => {
			if (isSolanaError(e, 8190003)) pingerAbortController.abort();
		});
	}
	...
	const pingerAbortController = new o$2();
	...
	if (globalThis.navigator.onLine) restartPingTimer();
	...
	return {
		...channel,
		send(...args) {
			if (!pingerAbortController.signal.aborted) restartPingTimer();
...
function getChannelPoolingChannelCreator(createChannel, { maxSubscriptionsPerChannel, minChannels }) {
	...
	function recomputeFreeChannelIndex() {
		...
		for (let ii = 0; ii < pool.entries.length; ii++) {
			...
			if (nextPoolEntry.subscriptionCount < maxSubscriptionsPerChannel && (!mostFreeChannel || mostFreeChannel.subscriptionCount >= nextPoolEntry.subscriptionCount)) mostFreeChannel = {
	...
	return function getExistingChannelWithMostCapacityOrCreateChannel({ abortSignal }) {
		...
		} else poolEntry = pool.entries[pool.freeChannelIndex];
		poolEntry.subscriptionCount++;
		abortSignal.addEventListener("abort", function destroyConsumer() {
			poolEntry.subscriptionCount--;
			if (poolEntry.subscriptionCount === 0) destroyPoolEntry();
...
function getRpcSubscriptionsTransportWithSubscriptionCoalescing(transport) {
	...
	return function rpcSubscriptionsTransportWithSubscriptionCoalescing(config) {
		...
		signal.addEventListener("abort", () => {
			cachedDataPublisherPromise.numSubscribers--;
			if (cachedDataPublisherPromise.numSubscribers === 0) queueMicrotask(() => {
...
function createRpcSubscriptionsTransportFromChannelCreator(createChannel) {
	return (async ({ execute, signal }) => {
		return await execute({
			channel: await createChannel({ abortSignal: signal }),
			signal
		});
	});
}
function createSolanaRpcSubscriptionsImpl(clusterUrl, config) {
	return createSolanaRpcSubscriptionsFromTransport(createDefaultRpcSubscriptionsTransport({ createChannel: createDefaultSolanaRpcSubscriptionsChannelCreator({
		...config,
		url: clusterUrl
	}) }));
}
...
//#endregion
//#region ../node_modules/@solana/signers/dist/index.browser.mjs
function deduplicateSigners(signers) {
	const deduplicated = {};
	signers.forEach((signer) => {
		if (!deduplicated[signer.address]) deduplicated[signer.address] = signer;
		else if (!signersAreEquivalent(deduplicated[signer.address], signer)) throw new SolanaError(SOLANA_ERROR__SIGNER__ADDRESS_CANNOT_HAVE_MULTIPLE_SIGNERS, { address: signer.address });
	});
...
function signersAreEquivalent(a, b) {
	...
	return aKeys.every((key) => {
		...
		const aVal = a[key];
		const bVal = b[key];
		if (typeof aVal === "function" && typeof bVal === "function") return aVal.toString() === bVal.toString();
		return aVal === bVal;
	});
}
...
function assertIsTransactionModifyingSigner(value) {
	if (!isTransactionModifyingSigner(value)) throw new SolanaError(SOLANA_ERROR__SIGNER__EXPECTED_TRANSACTION_MODIFYING_SIGNER, { address: value.address });
}
function isTransactionPartialSigner(value) {
	return "signTransactions" in value && typeof value.signTransactions === "function";
}
function assertIsTransactionPartialSigner(value) {
	if (!isTransactionPartialSigner(value)) throw new SolanaError(SOLANA_ERROR__SIGNER__EXPECTED_TRANSACTION_PARTIAL_SIGNER, { address: value.address });
}
function isTransactionSendingSigner(value) {
	return "signAndSendTransactions" in value && typeof value.signAndSendTransactions === "function";
}
function assertIsTransactionSendingSigner(value) {
	if (!isTransactionSendingSigner(value)) throw new SolanaError(SOLANA_ERROR__SIGNER__EXPECTED_TRANSACTION_SENDING_SIGNER, { address: value.address });
}
...
function assertIsTransactionSigner(value) {
	if (!isTransactionSigner(value)) throw new SolanaError(SOLANA_ERROR__SIGNER__EXPECTED_TRANSACTION_SIGNER, { address: value.address });
}
...
function addSignersToInstruction(signers, instruction) {
	...
	return Object.freeze({
		...
		accounts: instruction.accounts.map((account) => {
			...
			if (!isSignerRole(account.role) || "signer" in account || !signer) return account;
			return Object.freeze({
				...account,
				signer
			});
...
function assertIsMessagePartialSigner(value) {
	if (!isMessagePartialSigner(value)) throw new SolanaError(SOLANA_ERROR__SIGNER__EXPECTED_MESSAGE_PARTIAL_SIGNER, { address: value.address });
}
...
function assertIsKeyPairSigner(value) {
	if (!isKeyPairSigner(value)) throw new SolanaError(SOLANA_ERROR__SIGNER__EXPECTED_KEY_PAIR_SIGNER, { address: value.address });
}
...
async function grindKeyPairSigner(config) {
	const [signer] = await grindKeyPairSigners({
		...config,
		amount: 1
	});
	return signer;
...
function assertIsMessageModifyingSigner(value) {
	if (!isMessageModifyingSigner(value)) throw new SolanaError(SOLANA_ERROR__SIGNER__EXPECTED_MESSAGE_MODIFYING_SIGNER, { address: value.address });
}
function isMessageSigner(value) {
	return isMessagePartialSigner(value) || isMessageModifyingSigner(value);
}
function assertIsMessageSigner(value) {
	if (!isMessageSigner(value)) throw new SolanaError(SOLANA_ERROR__SIGNER__EXPECTED_MESSAGE_SIGNER, { address: value.address });
}
...
function getSignersFromOffchainMessage({ requiredSignatories }) {
	return deduplicateSigners(requiredSignatories.filter(isMessageSigner));
}
...
function assertContainsResolvableTransactionSendingSigner(signers) {
	...
	if (sendingSigners.length === 0) throw new SolanaError(SOLANA_ERROR__SIGNER__TRANSACTION_SENDING_SIGNER_MISSING);
	if (sendingSigners.filter((signer) => !isTransactionPartialSigner(signer) && !isTransactionModifyingSigner(signer)).length > 1) throw new SolanaError(SOLANA_ERROR__SIGNER__TRANSACTION_CANNOT_HAVE_MULTIPLE_SENDING_SIGNERS);
}
...
async function signAndSendTransactionMessageWithSigners(transaction, config) {
	return await signAndSendTransactionWithSigners(getSignersFromTransactionMessage(transaction).filter(isTransactionSigner), compileTransaction(transaction), config);
}
...
async function signAndSendTransactionWithSigners(signers, transaction, config) {
	...
	const { partialSigners, modifyingSigners, sendingSigner } = categorizeTransactionSigners(deduplicateSigners(signers));
	abortSignal?.throwIfAborted();
	...
	if (!sendingSigner) throw new SolanaError(SOLANA_ERROR__SIGNER__TRANSACTION_SENDING_SIGNER_MISSING);
	abortSignal?.throwIfAborted();
...
var o$1 = globalThis.TextEncoder;
function createSignableMessage(content, signatures = {}) {
	return Object.freeze({
		content: typeof content === "string" ? new o$1().encode(content) : content,
		signatures: Object.freeze({ ...signatures })
	});
}
...
//#endregion
//#region ../node_modules/@solana/transaction-introspection/dist/index.browser.mjs
var EMPTY_LOADED_ADDRESSES = {
	readonly: [],
	writable: []
};
function getLoadedAddresses(meta) {
	return meta?.loadedAddresses ?? EMPTY_LOADED_ADDRESSES;
}
...
function decodeFromBase64(rpcTx) {
	...
	return {
		compiledMessage,
		loadedAddresses: getLoadedAddresses(rpcTx.meta),
		transaction
	};
}
function decodeFromBase58(rpcTx) {
	const [b58] = rpcTx.transaction;
	const { compiledMessage, transaction } = decodeFromWire(getBase58Encoder$1().encode(b58));
	return {
		compiledMessage,
		loadedAddresses: getLoadedAddresses(rpcTx.meta),
		transaction
...
function decodeFromJson(rpcTx) {
	const base58 = getBase58Encoder$1();
	...
	switch (version) {
		...
		default: throw new SolanaError(SOLANA_ERROR__TRANSACTION__VERSION_NUMBER_NOT_SUPPORTED, { unsupportedVersion: version });
	}
	return {
		compiledMessage,
		loadedAddresses: getLoadedAddresses(rpcTx.meta)
...
function decodeTransactionFromRpcResponse(rpcTx) {
	...
	if (isJsonParsedResponse(tx)) throw new SolanaError(SOLANA_ERROR__TRANSACTION_INTROSPECTION__CANNOT_DECODE_JSON_PARSED_TRANSACTION);
...
function getInnerInstructionsFromMeta(meta, accountMetas) {
	...
	const base58 = getBase58Encoder$1();
	const result = [];
	for (const group of meta.innerInstructions) for (let innerIndex = 0; innerIndex < group.instructions.length; innerIndex++) {
		const ix = group.instructions[innerIndex];
		const programMeta = accountMetas[ix.programIdIndex];
		if (!programMeta) throw new SolanaError(SOLANA_ERROR__TRANSACTION__FAILED_TO_DECOMPILE_INSTRUCTION_PROGRAM_ADDRESS_NOT_FOUND, { index: ix.programIdIndex });
		const accounts = ix.accounts.map((i) => {
			const accountMeta = accountMetas[i];
			if (!accountMeta) throw new SolanaError(SOLANA_ERROR__TRANSACTION__FAILED_TO_DECOMPILE_INSTRUCTION_ACCOUNT_INDEX_OUT_OF_RANGE, { index: i });
			return accountMeta;
		});
...
function getAccountMetasFromCompiledTransactionMessage(compiledMessage, loadedAddresses) {
	...
	const metas = [];
	let i = 0;
	for (let n = 0; n < numWritableSignerAccounts; n++, i++) metas.push({
		address: staticAccounts[i],
		role: AccountRole.WRITABLE_SIGNER
	});
	for (let n = 0; n < header.numReadonlySignerAccounts; n++, i++) metas.push({
		address: staticAccounts[i],
		role: AccountRole.READONLY_SIGNER
	});
	for (let n = 0; n < numWritableNonSignerAccounts; n++, i++) metas.push({
		address: staticAccounts[i],
		role: AccountRole.WRITABLE
	});
	for (let n = 0; n < header.numReadonlyNonSignerAccounts; n++, i++) metas.push({
		address: staticAccounts[i],
		role: AccountRole.READONLY
	});
	if (loadedAddresses) {
		for (const address of loadedAddresses.writable) metas.push({
			address,
			role: AccountRole.WRITABLE
		});
		for (const address of loadedAddresses.readonly) metas.push({
			address,
			role: AccountRole.READONLY
		});
	}
...
function normalizeCompiledInstructions(compiledMessage) {
	if (compiledMessage.version === "legacy" || compiledMessage.version === 0) return compiledMessage.instructions.map((ix) => ({
		accountIndices: ix.accountIndices ?? [],
		data: ix.data ?? /* @__PURE__ */ new Uint8Array(),
	...
	throw new SolanaError(SOLANA_ERROR__TRANSACTION__VERSION_NUMBER_NOT_SUPPORTED, { unsupportedVersion: compiledMessage.version });
}
function resolveInstruction(ix, metas) {
	const programMeta = metas[ix.programAddressIndex];
	if (!programMeta) throw new SolanaError(SOLANA_ERROR__TRANSACTION__FAILED_TO_DECOMPILE_INSTRUCTION_PROGRAM_ADDRESS_NOT_FOUND, { index: ix.programAddressIndex });
...
function walkInstructions(args) {
	...
	if (meta) for (const inner of getInnerInstructionsFromMeta(meta, accountMetas)) {
		if (inner.trace.kind !== "inner") continue;
		const group = innerByOuterIndex.get(inner.trace.outerIndex);
		if (group) group.push(inner);
		else innerByOuterIndex.set(inner.trace.outerIndex, [inner]);
	}
	const result = [];
	...
	for (const group of innerByOuterIndex.values()) result.push(...group);
	return result;
}
//#endregion
//#region ../node_modules/@solana/transaction-confirmation/dist/index.browser.mjs
...
function createBlockHeightExceedencePromiseFactory({ rpc, rpcSubscriptions }) {
	return async function getBlockHeightExceedencePromise({ abortSignal: callerAbortSignal, commitment, lastValidBlockHeight }) {
		...
		try {
			const [slotNotifications, { blockHeight: initialBlockHeight, differenceBetweenSlotHeightAndBlockHeight }] = await Promise.all([rpcSubscriptions.slotNotifications().subscribe({ abortSignal: abortController.signal }), getBlockHeightAndDifferenceBetweenSlotHeightAndBlockHeight()]);
			callerAbortSignal.throwIfAborted();
			...
			if (currentBlockHeight <= lastValidBlockHeight) {
				...
				for await (const slotNotification of slotNotifications) {
					...
					if (slot - lastKnownDifferenceBetweenSlotHeightAndBlockHeight > lastValidBlockHeight) {
						...
						if (currentBlockHeight > lastValidBlockHeight) break;
						else lastKnownDifferenceBetweenSlotHeightAndBlockHeight = currentDifferenceBetweenSlotHeightAndBlockHeight;
					}
				}
			}
			callerAbortSignal.throwIfAborted();
			throw new SolanaError(1, {
				currentBlockHeight,
...
function createNonceInvalidationPromiseFactory({ rpc, rpcSubscriptions }) {
	return async function getNonceInvalidationPromise({ abortSignal: callerAbortSignal, commitment, currentNonceValue: expectedNonceValue, nonceAccountAddress }) {
		...
		function getNonceFromAccountData([base64EncodedBytes]) {
			const nonceValueBytes = base64Encoder.encode(base64EncodedBytes).slice(NONCE_VALUE_OFFSET, NONCE_VALUE_OFFSET + 32);
			return base58Decoder.decode(nonceValueBytes);
		}
		const nonceAccountDidAdvancePromise = (async () => {
			for await (const accountNotification of accountNotifications) {
				const nonceValue = getNonceFromAccountData(accountNotification.value.data);
				if (nonceValue !== expectedNonceValue) throw new SolanaError(2, {
		...
		const nonceIsAlreadyInvalidPromise = (async () => {
			...
			if (!nonceAccount) throw new SolanaError(3, { nonceAccountAddress });
...
function createRecentSignatureConfirmationPromiseFactory({ rpc, rpcSubscriptions }) {
	return async function getRecentSignatureConfirmationPromise({ abortSignal: callerAbortSignal, commitment, signature }) {
		...
		const signatureDidCommitPromise = (async () => {
			for await (const signatureStatusNotification of signatureStatusNotifications) if (signatureStatusNotification.value.err) throw getSolanaErrorFromTransactionError(signatureStatusNotification.value.err);
			else return;
		})();
		const signatureStatusLookupPromise = (async () => {
			const { value: signatureStatusResults } = await rpc.getSignatureStatuses([signature]).send({ abortSignal: abortController.signal });
			const signatureStatus = signatureStatusResults[0];
			if (signatureStatus?.err) throw getSolanaErrorFromTransactionError(signatureStatus.err);
			else if (signatureStatus?.confirmationStatus && commitmentComparator(signatureStatus.confirmationStatus, commitment) >= 0) return;
			else await new Promise(() => {});
		})();
		try {
	...
	try {
		return await new Promise((_, reject) => {
			const handleAbort = (e) => {
				...
				reject(new DOMException(e.target.reason, "AbortError"));
			};
			callerAbortSignal.addEventListener("abort", handleAbort, { signal: abortController.signal });
...
async function raceStrategies(signature, config, getSpecificStrategiesForRace) {
	...
	try {
		...
		return await safeRace([getRecentSignatureConfirmationPromise({
			abortSignal: abortController.signal,
			commitment,
			signature
		}), ...specificStrategies]);
...
async function waitForDurableNonceTransactionConfirmation(config) {
	await raceStrategies(getSignatureFromTransaction(config.transaction), config, function getSpecificStrategiesForRace({ abortSignal, commitment, getNonceInvalidationPromise, transaction }) {
		return [getNonceInvalidationPromise({
			abortSignal,
			commitment,
...
async function waitForRecentTransactionConfirmation(config) {
	await raceStrategies(getSignatureFromTransaction(config.transaction), config, function getSpecificStrategiesForRace({ abortSignal, commitment, getBlockHeightExceedencePromise, transaction }) {
		return [getBlockHeightExceedencePromise({
			abortSignal,
			commitment,
...
//#endregion
//#region ../node_modules/@solana/kit/dist/index.browser.mjs
async function* createAsyncGeneratorWithInitialValueAndSlotTracking({ abortSignal, rpcRequest, rpcValueMapper, rpcSubscriptionRequest, rpcSubscriptionValueMapper }) {
	if (abortSignal.aborted) return;
	...
	rpcSubscriptionRequest.subscribe({ abortSignal: signal }).then(async (notifications) => {
		for await (const { context: { slot }, value } of notifications) {
			if (signal.aborted) return;
...
function createReactiveStoreWithInitialValueAndSlotTracking({ initialValueMapper, initialValueSource, streamSource, streamValueMapper }) {
	...
	function performConnect(callerSignal) {
		...
		function handleSlottedValue({ context: { slot }, value }) {
			...
			if (slot < lastUpdateSlot) {
				if (currentState.data !== void 0) setState({
					data: currentState.data,
					error: void 0,
					status: "loaded"
				});
		...
		const unsubscribeAction = actionStore.subscribe(() => {
			...
			if (state.status === "success") {
				...
				handleSlottedValue({
					context,
					value: initialValueMapper(value)
				});
			} else if (state.status === "error") handleError(state.error);
		});
...
function estimateComputeUnitLimitFactory({ rpc }) {
	return async function estimateComputeUnitLimit(transactionMessage, config) {
		...
		const wireTransactionBytes = getBase64EncodedWireTransaction(pipe(transactionMessage, (m) => setTransactionMessageComputeUnitLimit(MAX_COMPUTE_UNIT_LIMIT, m), compileTransaction));
		try {
			const { err: transactionError, ...simulationResult } = (await rpc.simulateTransaction(wireTransactionBytes, {
				...simulateConfig,
				encoding: "base64",
				replaceRecentBlockhash,
				sigVerify: false
			}).send({ abortSignal })).value;
			if (simulationResult.unitsConsumed == null) throw new SolanaError(SOLANA_ERROR__TRANSACTION__FAILED_TO_ESTIMATE_COMPUTE_LIMIT);
			if (transactionError) throw new SolanaError(SOLANA_ERROR__TRANSACTION__FAILED_WHEN_SIMULATING_TO_ESTIMATE_COMPUTE_LIMIT, {
				...simulationResult,
			...
			if (isSolanaError(e, 5663019)) throw e;
...
function estimateAndSetComputeUnitLimitFactory(estimateComputeUnitLimit) {
	return async function estimateAndSetComputeUnitLimit(transactionMessage, config) {
		...
		if (existingLimit && existingLimit !== MAX_COMPUTE_UNIT_LIMIT) return transactionMessage;
		return setTransactionMessageComputeUnitLimit(await estimateComputeUnitLimit(transactionMessage, config), transactionMessage);
	};
}
function fillTransactionMessageProvisoryComputeUnitLimit(transactionMessage) {
	if (getTransactionMessageComputeUnitLimit(transactionMessage) !== void 0) return transactionMessage;
...
async function fetchAddressesForLookupTables(lookupTableAddresses, rpc, config) {
	...
	const fetchedLookupTables = await fetchJsonParsedAccounts(rpc, lookupTableAddresses, config);
...
async function decompileTransactionMessageFetchingLookupTables(compiledTransactionMessage, rpc, config) {
	const lookupTableAddresses = ("addressTableLookups" in compiledTransactionMessage && compiledTransactionMessage.addressTableLookups !== void 0 && compiledTransactionMessage.addressTableLookups.length > 0 ? compiledTransactionMessage.addressTableLookups : []).map((l) => l.lookupTableAddress);
	const { lastValidBlockHeight, ...fetchAccountsConfig } = config ?? {};
	return decompileTransactionMessage4(compiledTransactionMessage, {
		addressesByLookupTableAddress: lookupTableAddresses.length > 0 ? await fetchAddressesForLookupTables(lookupTableAddresses, rpc, fetchAccountsConfig) : {},
		lastValidBlockHeight
	});
}
...
function estimateResourceLimitsFactory({ rpc }) {
	return async function estimateResourceLimits(transactionMessage, config) {
		...
		const wireTransactionBytes = getBase64EncodedWireTransaction(pipe(transactionMessage, (m) => setTransactionMessageComputeUnitLimit(MAX_COMPUTE_UNIT_LIMIT2, m), (m) => isDataSizeRequired ? setTransactionMessageLoadedAccountsDataSizeLimit(MAX_LOADED_ACCOUNTS_DATA_SIZE_LIMIT, m) : m, compileTransaction));
		try {
			const { err: transactionError, ...simulationResult } = (await rpc.simulateTransaction(wireTransactionBytes, {
				...simulateConfig,
				encoding: "base64",
			...
			if (simulationResult.unitsConsumed == null) throw new SolanaError(SOLANA_ERROR__TRANSACTION__FAILED_TO_ESTIMATE_COMPUTE_LIMIT);
			if (isDataSizeRequired && simulationResult.loadedAccountsDataSize == null) throw new SolanaError(SOLANA_ERROR__TRANSACTION__FAILED_TO_ESTIMATE_LOADED_ACCOUNTS_DATA_SIZE_LIMIT);
			...
			if (isDataSizeRequired) return {
				computeUnitLimit,
				loadedAccountsDataSizeLimit: simulationResult.loadedAccountsDataSize
			};
			return simulationResult.loadedAccountsDataSize == null ? { computeUnitLimit } : {
				computeUnitLimit,
				loadedAccountsDataSizeLimit: simulationResult.loadedAccountsDataSize
			};
		} catch (e) {
			if (isSolanaError(e, 5663037) || isSolanaError(e, 5663036)) throw e;
			throw new SolanaError(SOLANA_ERROR__TRANSACTION__FAILED_TO_ESTIMATE_COMPUTE_LIMIT, { cause: e });
		}
	};
}
function estimateAndSetResourceLimitsFactory(estimateResourceLimits) {
	return async function estimateAndSetResourceLimits(transactionMessage, config) {
		...
		if (computeUnitLimitIsExplicit && loadedAccountsDataSizeLimitIsExplicit) return transactionMessage;
		const estimate = await estimateResourceLimits(transactionMessage, config);
		let result = transactionMessage;
		if (!computeUnitLimitIsExplicit) result = setTransactionMessageComputeUnitLimit(estimate.computeUnitLimit, result);
		if (isV1 && !loadedAccountsDataSizeLimitIsExplicit && "loadedAccountsDataSizeLimit" in estimate) result = setTransactionMessageLoadedAccountsDataSizeLimit(estimate.loadedAccountsDataSizeLimit, result);
		return result;
	};
}
function fillTransactionMessageProvisoryResourceLimits(transactionMessage) {
	let result = transactionMessage;
	if (getTransactionMessageComputeUnitLimit(result) === void 0) result = setTransactionMessageComputeUnitLimit(PROVISORY_LIMIT, result);
	if (result.version === 1 && getTransactionMessageLoadedAccountsDataSizeLimit(result) === void 0) result = setTransactionMessageLoadedAccountsDataSizeLimit(PROVISORY_LIMIT, result);
	return result;
}
function getSendTransactionConfigWithAdjustedPreflightCommitment(commitment, config) {
	if (!config?.preflightCommitment && commitmentComparator(commitment, "finalized") < 0) return {
		...config,
...
function sendAndConfirmDurableNonceTransactionFactory({ rpc, rpcSubscriptions }) {
		...
		return async function wrappedGetNonceInvalidationPromise(config) {
			try {
				...
				if (isSolanaError(e, 2)) {
					...
					if (status === null || status === void 0) throw e;
					if (status.confirmationStatus !== null && commitmentComparator(status.confirmationStatus, config.commitment) >= 0) {
						if (status.err !== null) throw getSolanaErrorFromTransactionError(status.err);
						return;
					}
	...
	async function confirmDurableNonceTransaction(config) {
		const wrappedGetNonceInvalidationPromise = createNonceInvalidationPromiseHandlingRaceCondition(getSignatureFromTransaction(config.transaction));
...
//#endregion
export { partiallySignTransactionMessageWithSigners as $, lamports as $a, getI32Decoder as $c, SOLANA_ERROR__CODECS__INVALID_PATTERN_MATCH_BYTES as $d, SOLANA_ERROR__INSTRUCTION_ERROR__INVALID_ARGUMENT as $f, SOLANA_ERROR__TRANSACTION__NONCE_ACCOUNT_CANNOT_BE_IN_LOOKUP_TABLE as $g, SOLANA_ERROR__TRANSACTION_ERROR__INVALID_ADDRESS_LOOKUP_TABLE_DATA as $h, compressTransactionMessageUsingAddressLookupTables as $i, parseJsonRpcAccount as $l, SOLANA_ERROR__PROGRAM_CLIENTS__INSUFFICIENT_ACCOUNT_METAS as $m, createTransactionPlanExecutor as $n, formatBinaryFixedPoint as $o, SOLANA_ERROR__JSON_RPC__PARSE_ERROR as $p, transformInstructionPlan as $r, getEnumCodec as $s, assertIsOffchainMessageUtf8Of1232BytesMax as $t, padBytes as $u, assertIsTransactionSendingSigner as A, assertIsUnixTimestamp as Aa, getTupleDecoder as Ac, SOLANA_ERROR__ADDRESSES__PDA_BUMP_SEED_OUT_OF_RANGE as Ad, SOLANA_ERROR__INSTRUCTION_ERROR__ACCOUNT_NOT_RENT_EXEMPT as Af, SOLANA_ERROR__TRANSACTION__EXPECTED_BLOCKHASH_LIFETIME as Ag, SOLANA_ERROR__SUBTLE_CRYPTO__CANNOT_EXPORT_NON_EXTRACTABLE_KEY as Ah, assertIsSignature as Ai, getAddressCodec as Al, SOLANA_ERROR__MALFORMED_JSON_RPC_ERROR as Am, verifyOffchainMessageEnvelope as An, some as Ao, SOLANA_ERROR__INSTRUCTION_PLANS__MAX_INSTRUCTIONS_PER_TRANSACTION_EXCEEDED as Ap, isSingleTransactionPlanResult as Ar, toSignedDecimalFixedPoint as As, createSolanaRpc as At, assertByteArrayHasEnoughBytesForCodec as Au, getSignersFromTransactionMessage as B, getDefaultLamportsEncoder as Ba, getF32Codec as Bc, SOLANA_ERROR__CODECS__ENCODER_DECODER_SIZE_COMPATIBILITY_MISMATCH as Bd, SOLANA_ERROR__INSTRUCTION_ERROR__EXECUTABLE_DATA_MODIFIED as Bf, SOLANA_ERROR__TRANSACTION__FAILED_WHEN_SIMULATING_TO_ESTIMATE_RESOURCE_LIMITS as Bg, SOLANA_ERROR__TRANSACTION_ERROR__ACCOUNT_IN_USE as Bh, isSignatureBytes as Bi, offCurveAddress as Bl, SOLANA_ERROR__OFFCHAIN_MESSAGE__MESSAGE_MUST_BE_NON_EMPTY as Bm, assertIsParallelTransactionPlanResult as Bn, binaryFixedPoint as Bo, SOLANA_ERROR__INVALID_BLOCKHASH_BYTE_LENGTH as Bp, parallelTransactionPlanResult as Br, getBitArrayEncoder as Bs, demultiplexDataPublisher as Bt, createDecoderThatConsumesEntireByteArray as Bu, assertIsKeyPairSigner as C, setTransactionMessageLifetimeUsingDurableNonce as Ca, getSetCodec as Cc, SOLANA_ERROR__ADDRESSES__INVALID_BYTE_LENGTH as Cd, SOLANA_ERROR__FS__UNSUPPORTED_ENVIRONMENT as Cf, SOLANA_ERROR__TRANSACTION_INTROSPECTION__UNRECOGNIZED_GET_TRANSACTION_RESPONSE as Cg, SOLANA_ERROR__SIGNER__EXPECTED_TRANSACTION_SENDING_SIGNER as Ch, isSendableTransaction as Ci, getU8Decoder as Cl, SOLANA_ERROR__KEYS__INVALID_PRIVATE_KEY_BYTE_LENGTH as Cm, isOffchainMessageContentUtf8Of65535BytesMax as Cn, getOptionCodec as Co, SOLANA_ERROR__INSTRUCTION_ERROR__UNSUPPORTED_PROGRAM_ID as Cp, isParallelTransactionPlan as Cr, rawBinaryFixedPoint as Cs, createRpcSubscriptionsApi as Ct, removeNullCharacters as Cu, assertIsTransactionMessageWithSingleSendingSigner as D, assertIsLamports as Da, getStructDecoder as Dc, SOLANA_ERROR__ADDRESSES__MALFORMED_PDA as Dd, SOLANA_ERROR__INSTRUCTION_ERROR__ACCOUNT_DATA_SIZE_CHANGED as Df, SOLANA_ERROR__TRANSACTION__CANNOT_ENCODE_WITH_EMPTY_MESSAGE_BYTES as Dg, SOLANA_ERROR__SIGNER__WALLET_ACCOUNT_CANNOT_SIGN_TRANSACTION as Dh, isTransactionWithinSizeLimit as Di, assertIsOffCurveAddress as Dl, SOLANA_ERROR__KEYS__WRITE_KEY_PAIR_UNSUPPORTED_ENVIRONMENT as Dm, offchainMessageContentUtf8Of65535BytesMax as Dn, isOption as Do, SOLANA_ERROR__INSTRUCTION_PLANS__FAILED_SINGLE_TRANSACTION_PLAN_RESULT_NOT_FOUND as Dp, isSequentialTransactionPlanResult as Dr, subtractBinaryFixedPoint as Ds, transformChannelOutboundMessages as Dt, addDecoderSizePrefix as Du, assertIsMessageSigner as E, assertIsBlockhash as Ea, getStructCodec as Ec, SOLANA_ERROR__ADDRESSES__INVALID_SEEDS_POINT_ON_CURVE as Ed, SOLANA_ERROR__INSTRUCTION_ERROR__ACCOUNT_BORROW_OUTSTANDING as Ef, SOLANA_ERROR__TRANSACTION__CANNOT_DECODE_EMPTY_TRANSACTION_BYTES as Eg, SOLANA_ERROR__SIGNER__TRANSACTION_SENDING_SIGNER_MISSING as Eh, isTransactionWithDurableNonceLifetime as Ei, assertIsAddress as El, SOLANA_ERROR__KEYS__SIGNATURE_STRING_LENGTH_OUT_OF_RANGE as Em, offchainMessageContentUtf8Of1232BytesMax as En, isNone as Eo, SOLANA_ERROR__INSTRUCTION_PLANS__EXPECTED_SUCCESSFUL_TRANSACTION_PLAN_RESULT as Ep, isSequentialTransactionPlan as Er, rescaleDecimalFixedPoint as Es, transformChannelInboundMessages as Et, addDecoderSentinel as Eu, createSignableMessage as F, getBlockhashComparator as Fa, getUnitCodec as Fc, SOLANA_ERROR__CODECS__CANNOT_DECODE_EMPTY_BYTE_ARRAY as Fd, SOLANA_ERROR__INSTRUCTION_ERROR__COMPUTATIONAL_BUDGET_EXCEEDED as Ff, SOLANA_ERROR__TRANSACTION__FAILED_TO_DECOMPILE_INSTRUCTION_ACCOUNT_INDEX_OUT_OF_RANGE as Fg, SOLANA_ERROR__SUBTLE_CRYPTO__GENERATE_FUNCTION_UNIMPLEMENTED as Fh, generateKeyPair as Fi, getProgramDerivedAddress as Fl, SOLANA_ERROR__OFFCHAIN_MESSAGE__ENVELOPE_SIGNERS_MISMATCH as Fm, assertIsNonDivisibleSequentialInstructionPlan as Fn, absoluteDecimalFixedPoint as Fo, SOLANA_ERROR__INSTRUCTION_PLANS__UNEXPECTED_TRANSACTION_PLAN as Fp, nonDivisibleSequentialInstructionPlan as Fr, getArrayCodec as Fs, isJsonRpcPayload as Ft, bytesEqual as Fu, isMessagePartialSigner as G, getSolDecoder as Ga, getF64Encoder as Gc, SOLANA_ERROR__CODECS__EXPECTED_VARIABLE_LENGTH as Gd, SOLANA_ERROR__INSTRUCTION_ERROR__GENERIC_ERROR as Gf, SOLANA_ERROR__TRANSACTION__INVALID_CONFIG_VALUE_KIND as Gg, SOLANA_ERROR__TRANSACTION_ERROR__BLOCKHASH_NOT_FOUND as Gh, writeKeyPair as Gi, assertAccountsExist as Gl, SOLANA_ERROR__OFFCHAIN_MESSAGE__SIGNATORIES_MUST_BE_SORTED as Gm, assertIsSingleTransactionPlan as Gn, cmpDecimalFixedPoint as Go, SOLANA_ERROR__INVARIANT_VIOLATION__INVALID_TRANSACTION_PLAN_KIND as Gp, sequentialInstructionPlan as Gr, getBytesDecoder as Gs, withCleanup as Gt, fixEncoderSize as Gu, grindKeyPairSigners as H, getLamportsDecoder as Ha, getF32Encoder as Hc, SOLANA_ERROR__CODECS__EXPECTED_DECODER_TO_CONSUME_ENTIRE_BYTE_ARRAY as Hd, SOLANA_ERROR__INSTRUCTION_ERROR__EXECUTABLE_MODIFIED as Hf, SOLANA_ERROR__TRANSACTION__FEE_PAYER_SIGNATURE_MISSING as Hg, SOLANA_ERROR__TRANSACTION_ERROR__ACCOUNT_NOT_FOUND as Hh, signature as Hi, assertAccountDecoded as Hl, SOLANA_ERROR__OFFCHAIN_MESSAGE__NUM_REQUIRED_SIGNERS_CANNOT_BE_ZERO as Hm, assertIsSequentialTransactionPlan as Hn, binaryFixedPointToNumber as Ho, SOLANA_ERROR__INVARIANT_VIOLATION__CACHED_ABORTABLE_ITERABLE_CACHE_ENTRY_MISSING as Hp, parseInstructionPlanInput as Hr, getBooleanDecoder as Hs, isProgramError as Ht, fixBytes as Hu, createSignerFromKeyPair as I, getBlockhashDecoder as Ia, getUnitDecoder as Ic, SOLANA_ERROR__CODECS__CANNOT_USE_LEXICAL_VALUES_AS_ENUM_DISCRIMINATORS as Id, SOLANA_ERROR__INSTRUCTION_ERROR__CUSTOM as If, SOLANA_ERROR__TRANSACTION__FAILED_TO_DECOMPILE_INSTRUCTION_PROGRAM_ADDRESS_NOT_FOUND as Ig, SOLANA_ERROR__SUBTLE_CRYPTO__SIGN_FUNCTION_UNIMPLEMENTED as Ih, getPublicKeyFromPrivateKey as Ii, getPublicKeyFromAddress as Il, SOLANA_ERROR__OFFCHAIN_MESSAGE__INVALID_APPLICATION_DOMAIN_BYTE_LENGTH as Im, assertIsNonDivisibleSequentialTransactionPlan as In, addBinaryFixedPoint as Io, SOLANA_ERROR__INSTRUCTION_PLANS__UNEXPECTED_TRANSACTION_PLAN_RESULT as Ip, nonDivisibleSequentialTransactionPlan as Ir, getArrayDecoder as Is, createRpcMessage as It, combineCodec as Iu, isTransactionModifyingSigner as J, isLamports as Ja, getI128Encoder as Jc, SOLANA_ERROR__CODECS__INVALID_CONSTANT as Jd, SOLANA_ERROR__INSTRUCTION_ERROR__INCORRECT_AUTHORITY as Jf, SOLANA_ERROR__TRANSACTION__INVALID_NONCE_TRANSACTION_INSTRUCTIONS_MISSING as Jg, SOLANA_ERROR__TRANSACTION_ERROR__DUPLICATE_INSTRUCTION as Jh, appendTransactionMessageInstruction as Ji, fetchEncodedAccounts as Jl, SOLANA_ERROR__OFFCHAIN_MESSAGE__SIGNATURE_VERIFICATION_FAILURE as Jm, assertIsSuccessfulTransactionPlanResult as Jn, decimalFixedPointToString as Jo, SOLANA_ERROR__INVARIANT_VIOLATION__SWITCH_MUST_BE_EXHAUSTIVE as Jp, singleInstructionPlan as Jr, getConstantDecoder as Js, assertIsOffchainMessageApplicationDomain as Jt, isVariableSize as Ju, isMessageSigner as K, getSolEncoder as Ka, getI128Codec as Kc, SOLANA_ERROR__CODECS__EXPECTED_ZERO_VALUE_TO_MATCH_ITEM_FIXED_SIZE as Kd, SOLANA_ERROR__INSTRUCTION_ERROR__ILLEGAL_OWNER as Kf, SOLANA_ERROR__TRANSACTION__INVALID_NONCE_ACCOUNT_INDEX as Kg, SOLANA_ERROR__TRANSACTION_ERROR__CALL_CHAIN_TOO_DEEP as Kh, getAbortablePromise as Ki, decodeAccount as Kl, SOLANA_ERROR__OFFCHAIN_MESSAGE__SIGNATORIES_MUST_BE_UNIQUE as Km, assertIsSingleTransactionPlanResult as Kn, decimalFixedPoint as Ko, SOLANA_ERROR__INVARIANT_VIOLATION__SUBSCRIPTION_ITERATOR_MUST_NOT_POLL_BEFORE_RESOLVING_EXISTING_MESSAGE_PROMISE as Kp, sequentialTransactionPlan as Kr, getBytesEncoder as Ks, OffchainMessageContentFormat as Kt, getEncodedSize as Ku, generateKeyPairSigner as L, getBlockhashEncoder as La, getUnitEncoder as Lc, SOLANA_ERROR__CODECS__ENCODED_BYTES_MUST_NOT_INCLUDE_SENTINEL as Ld, SOLANA_ERROR__INSTRUCTION_ERROR__DUPLICATE_ACCOUNT_INDEX as Lf, SOLANA_ERROR__TRANSACTION__FAILED_TO_ESTIMATE_COMPUTE_LIMIT as Lg, SOLANA_ERROR__SUBTLE_CRYPTO__VERIFY_FUNCTION_UNIMPLEMENTED as Lh, grindKeyPair as Li, isAddress as Ll, SOLANA_ERROR__OFFCHAIN_MESSAGE__MAXIMUM_LENGTH_EXCEEDED as Lm, assertIsNonDivisibleSequentialTransactionPlanResult as Ln, addDecimalFixedPoint as Lo, SOLANA_ERROR__INSTRUCTION__EXPECTED_TO_HAVE_ACCOUNTS as Lp, nonDivisibleSequentialTransactionPlanResult as Lr, getArrayEncoder as Ls, createAsyncIterableFromDataPublisher as Lt, containsBytes as Lu, createKeyPairSignerFromBytes as M, commitmentComparator as Ma, getUnionCodec as Mc, SOLANA_ERROR__ADDRESSES__STRING_LENGTH_OUT_OF_RANGE as Md, SOLANA_ERROR__INSTRUCTION_ERROR__BORSH_IO_ERROR as Mf, SOLANA_ERROR__TRANSACTION__FAILED_TO_DECOMPILE_ADDRESS_LOOKUP_TABLE_CONTENTS_MISSING as Mg, SOLANA_ERROR__SUBTLE_CRYPTO__DISALLOWED_IN_INSECURE_CONTEXT as Mh, createKeyPairFromBytes as Mi, getAddressDecoder as Ml, SOLANA_ERROR__NONCE_ACCOUNT_NOT_FOUND as Mm, assertIsCanceledSingleTransactionPlanResult as Mn, unwrapOptionRecursively as Mo, SOLANA_ERROR__INSTRUCTION_PLANS__MESSAGE_PACKER_ALREADY_COMPLETE as Mp, isSuccessfulTransactionPlanResult as Mr, toUnsignedDecimalFixedPoint as Ms, createSolanaRpcApi as Mt, assertByteArrayOffsetIsNotOutOfRange as Mu, createKeyPairSignerFromPrivateKeyBytes as N, devnet as Na, getUnionDecoder as Nc, SOLANA_ERROR__BLOCKHASH_STRING_LENGTH_OUT_OF_RANGE as Nd, SOLANA_ERROR__INSTRUCTION_ERROR__BUILTIN_PROGRAMS_MUST_CONSUME_COMPUTE_UNITS as Nf, SOLANA_ERROR__TRANSACTION__FAILED_TO_DECOMPILE_ADDRESS_LOOKUP_TABLE_INDEX_OUT_OF_RANGE as Ng, SOLANA_ERROR__SUBTLE_CRYPTO__ED25519_ALGORITHM_UNIMPLEMENTED as Nh, createKeyPairFromPrivateKeyBytes as Ni, getAddressEncoder as Nl, SOLANA_ERROR__OFFCHAIN_MESSAGE__ADDRESSES_CANNOT_SIGN_OFFCHAIN_MESSAGE as Nm, assertIsFailedSingleTransactionPlanResult as Nn, wrapNullable as No, SOLANA_ERROR__INSTRUCTION_PLANS__NON_DIVISIBLE_TRANSACTION_PLANS_NOT_SUPPORTED as Np, isTransactionPlan as Nr, assertValidNumberOfItemsForCodec as Ns, createJsonRpcApi as Nt, assertIsFixedSize as Nu, assertIsTransactionModifyingSigner as O, assertIsStringifiedBigInt as Oa, getStructEncoder as Oc, SOLANA_ERROR__ADDRESSES__MAX_NUMBER_OF_PDA_SEEDS_EXCEEDED as Od, SOLANA_ERROR__INSTRUCTION_ERROR__ACCOUNT_DATA_TOO_SMALL as Of, SOLANA_ERROR__TRANSACTION__CANNOT_ENCODE_WITH_EMPTY_SIGNATURES as Og, SOLANA_ERROR__SIGNER__WALLET_MULTISIGN_UNIMPLEMENTED as Oh, partiallySignTransaction as Oi, assertIsProgramDerivedAddress as Ol, SOLANA_ERROR__LAMPORTS_OUT_OF_RANGE as Om, partiallySignOffchainMessageEnvelope as On, isSome as Oo, SOLANA_ERROR__INSTRUCTION_PLANS__FAILED_TO_EXECUTE_TRANSACTION_PLAN as Op, isSingleInstructionPlan as Or, subtractDecimalFixedPoint as Os, DEFAULT_RPC_CONFIG as Ot, addEncoderSentinel as Ou, createNoopSigner as P, getBlockhashCodec as Pa, getUnionEncoder as Pc, SOLANA_ERROR__BLOCK_HEIGHT_EXCEEDED as Pd, SOLANA_ERROR__INSTRUCTION_ERROR__CALL_DEPTH as Pf, SOLANA_ERROR__TRANSACTION__FAILED_TO_DECOMPILE_FEE_PAYER_MISSING as Pg, SOLANA_ERROR__SUBTLE_CRYPTO__EXPORT_FUNCTION_UNIMPLEMENTED as Ph, createPrivateKeyFromBytes as Pi, getAddressFromPublicKey as Pl, SOLANA_ERROR__OFFCHAIN_MESSAGE__APPLICATION_DOMAIN_STRING_LENGTH_OUT_OF_RANGE as Pm, assertIsMessagePackerInstructionPlan as Pn, absoluteBinaryFixedPoint as Po, SOLANA_ERROR__INSTRUCTION_PLANS__UNEXPECTED_INSTRUCTION_PLAN as Pp, isTransactionPlanResult as Pr, createDependentStructDecoder as Ps, createRpc as Pt, assertIsVariableSize as Pu, partiallySignOffchainMessageWithSigners as Q, isUnixTimestamp as Qa, getI32Codec as Qc, SOLANA_ERROR__CODECS__INVALID_NUMBER_OF_ITEMS as Qd, SOLANA_ERROR__INSTRUCTION_ERROR__INVALID_ACCOUNT_OWNER as Qf, SOLANA_ERROR__TRANSACTION__MESSAGE_SIGNATURES_MISMATCH as Qg, SOLANA_ERROR__TRANSACTION_ERROR__INVALID_ACCOUNT_INDEX as Qh, compileTransactionMessage4 as Qi, parseBase64RpcAccount as Ql, SOLANA_ERROR__PROGRAM_CLIENTS__FAILED_TO_IDENTIFY_INSTRUCTION as Qm, createFailedToSendTransactionsError as Qn, eqDecimalFixedPoint as Qo, SOLANA_ERROR__JSON_RPC__METHOD_NOT_FOUND as Qp, summarizeTransactionPlanResult as Qr, getDiscriminatedUnionEncoder as Qs, assertIsOffchainMessageRestrictedAsciiOf1232BytesMax as Qt, offsetEncoder as Qu, getSignersFromInstruction as R, getDefaultLamportsCodec as Ra, Endian as Rc, SOLANA_ERROR__CODECS__ENCODER_DECODER_FIXED_SIZE_MISMATCH as Rd, SOLANA_ERROR__INSTRUCTION_ERROR__DUPLICATE_ACCOUNT_OUT_OF_SYNC as Rf, SOLANA_ERROR__TRANSACTION__FAILED_TO_ESTIMATE_LOADED_ACCOUNTS_DATA_SIZE_LIMIT as Rg, SOLANA_ERROR__TIMESTAMP_OUT_OF_RANGE as Rh, grindKeyPairs as Ri, isOffCurveAddress as Rl, SOLANA_ERROR__OFFCHAIN_MESSAGE__MESSAGE_FORMAT_MISMATCH as Rm, assertIsParallelInstructionPlan as Rn, assertIsBinaryFixedPoint as Ro, SOLANA_ERROR__INSTRUCTION__EXPECTED_TO_HAVE_DATA as Rp, parallelInstructionPlan as Rr, getBitArrayCodec as Rs, createReactiveActionStore as Rt, createCodec as Ru, assertContainsResolvableTransactionSendingSigner as S, setTransactionMessageLifetimeUsingBlockhash as Sa, getPredicateEncoder as Sc, SOLANA_ERROR__ADDRESSES__INVALID_BASE58_ENCODED_ADDRESS as Sd, SOLANA_ERROR__FIXED_POINTS__VALUE_OUT_OF_RANGE as Sf, SOLANA_ERROR__TRANSACTION_INTROSPECTION__CANNOT_DECODE_JSON_PARSED_TRANSACTION as Sg, SOLANA_ERROR__SIGNER__EXPECTED_TRANSACTION_PARTIAL_SIGNER as Sh, isFullySignedTransaction as Si, getU8Codec as Sl, SOLANA_ERROR__KEYS__INVALID_KEY_PAIR_BYTE_LENGTH as Sm, isOffchainMessageContentUtf8Of1232BytesMax as Sn, pipe as So, SOLANA_ERROR__INSTRUCTION_ERROR__UNKNOWN as Sp, isParallelInstructionPlan as Sr, ratioDecimalFixedPoint as Ss, createSolanaRpcSubscriptionsApi_UNSTABLE as St, padNullCharacters as Su, assertIsMessagePartialSigner as T, setTransactionMessagePriorityFeeLamports as Ta, getSetEncoder as Tc, SOLANA_ERROR__ADDRESSES__INVALID_OFF_CURVE_ADDRESS as Td, SOLANA_ERROR__INSTRUCTION_ERROR__ACCOUNT_BORROW_FAILED as Tf, SOLANA_ERROR__TRANSACTION__ADDRESS_MISSING as Tg, SOLANA_ERROR__SIGNER__TRANSACTION_CANNOT_HAVE_MULTIPLE_SENDING_SIGNERS as Th, isTransactionWithBlockhashLifetime as Ti, address as Tl, SOLANA_ERROR__KEYS__PUBLIC_KEY_MUST_MATCH_PRIVATE_KEY as Tm, offchainMessageContentRestrictedAsciiOf1232BytesMax as Tn, getOptionEncoder as To, SOLANA_ERROR__INSTRUCTION_PLANS__EMPTY_INSTRUCTION_PLAN as Tp, isSequentialInstructionPlan as Tr, rescaleBinaryFixedPoint as Ts, executeRpcPubSubSubscriptionPlan as Tt, addCodecSizePrefix as Tu, isKeyPairSigner as U, getLamportsEncoder as Ua, getF64Codec as Uc, SOLANA_ERROR__CODECS__EXPECTED_FIXED_LENGTH as Ud, SOLANA_ERROR__INSTRUCTION_ERROR__EXTERNAL_ACCOUNT_DATA_MODIFIED as Uf, SOLANA_ERROR__TRANSACTION__INSTRUCTION_HEADERS_PAYLOADS_MISMATCH as Ug, SOLANA_ERROR__TRANSACTION_ERROR__ADDRESS_LOOKUP_TABLE_NOT_FOUND as Uh, signatureBytes as Ui, assertAccountExists as Ul, SOLANA_ERROR__OFFCHAIN_MESSAGE__NUM_SIGNATURES_MISMATCH as Um, assertIsSequentialTransactionPlanResult as Un, binaryFixedPointToString as Uo, SOLANA_ERROR__INVARIANT_VIOLATION__DATA_PUBLISHER_CHANNEL_UNIMPLEMENTED as Up, parseTransactionPlanInput as Ur, getBooleanEncoder as Us, createClient as Ut, fixCodecSize as Uu, grindKeyPairSigner as V, getLamportsCodec as Va, getF32Decoder as Vc, SOLANA_ERROR__CODECS__ENUM_DISCRIMINATOR_OUT_OF_RANGE as Vd, SOLANA_ERROR__INSTRUCTION_ERROR__EXECUTABLE_LAMPORT_CHANGE as Vf, SOLANA_ERROR__TRANSACTION__FEE_PAYER_MISSING as Vg, SOLANA_ERROR__TRANSACTION_ERROR__ACCOUNT_LOADED_TWICE as Vh, signBytes as Vi, BASE_ACCOUNT_SIZE as Vl, SOLANA_ERROR__OFFCHAIN_MESSAGE__NUM_ENVELOPE_SIGNATURES_CANNOT_BE_ZERO as Vm, assertIsSequentialInstructionPlan as Vn, binaryFixedPointToBase10 as Vo, SOLANA_ERROR__INVALID_NONCE as Vp, parseInstructionOrTransactionPlanInput as Vr, getBooleanCodec as Vs, getDataPublisherFromEventEmitter as Vt, createEncoder as Vu, isMessageModifyingSigner as W, getSolCodec as Wa, getF64Decoder as Wc, SOLANA_ERROR__CODECS__EXPECTED_POSITIVE_BYTE_LENGTH as Wd, SOLANA_ERROR__INSTRUCTION_ERROR__EXTERNAL_ACCOUNT_LAMPORT_SPEND as Wf, SOLANA_ERROR__TRANSACTION__INVALID_CONFIG_MASK_PRIORITY_FEE_BITS as Wg, SOLANA_ERROR__TRANSACTION_ERROR__ALREADY_PROCESSED as Wh, verifySignature as Wi, assertAccountsDecoded as Wl, SOLANA_ERROR__OFFCHAIN_MESSAGE__RESTRICTED_ASCII_BODY_CHARACTER_OUT_OF_RANGE as Wm, assertIsSingleInstructionPlan as Wn, cmpBinaryFixedPoint as Wo, SOLANA_ERROR__INVARIANT_VIOLATION__INVALID_INSTRUCTION_PLAN_KIND as Wp, passthroughFailedTransactionPlanExecution as Wr, getBytesCodec as Ws, extendClient as Wt, fixDecoderSize as Wu, isTransactionSendingSigner as X, isStringifiedBigInt as Xa, getI16Decoder as Xc, SOLANA_ERROR__CODECS__INVALID_ENUM_VARIANT as Xd, SOLANA_ERROR__INSTRUCTION_ERROR__INSUFFICIENT_FUNDS as Xf, SOLANA_ERROR__TRANSACTION__INVOKED_PROGRAMS_MUST_NOT_BE_WRITABLE as Xg, SOLANA_ERROR__TRANSACTION_ERROR__INSUFFICIENT_FUNDS_FOR_RENT as Xh, assertIsTransactionMessageWithBlockhashLifetime as Xi, fetchJsonParsedAccounts as Xl, SOLANA_ERROR__OFFCHAIN_MESSAGE__VERSION_NUMBER_NOT_SUPPORTED as Xm, createFailedToExecuteTransactionPlanError as Xn, divideDecimalFixedPoint as Xo, SOLANA_ERROR__JSON_RPC__INVALID_PARAMS as Xp, successfulSingleTransactionPlanResult as Xr, getDiscriminatedUnionCodec as Xs, assertIsOffchainMessageContentUtf8Of1232BytesMax as Xt, offsetCodec as Xu, isTransactionPartialSigner as Y, isSolanaRpcResponse as Ya, getI16Codec as Yc, SOLANA_ERROR__CODECS__INVALID_DISCRIMINATED_UNION_VARIANT as Yd, SOLANA_ERROR__INSTRUCTION_ERROR__INCORRECT_PROGRAM_ID as Yf, SOLANA_ERROR__TRANSACTION__INVOKED_PROGRAMS_CANNOT_PAY_FEES as Yg, SOLANA_ERROR__TRANSACTION_ERROR__INSUFFICIENT_FUNDS_FOR_FEE as Yh, appendTransactionMessageInstructions as Yi, fetchJsonParsedAccount as Yl, SOLANA_ERROR__OFFCHAIN_MESSAGE__UNEXPECTED_VERSION as Ym, canceledSingleTransactionPlanResult as Yn, divideBinaryFixedPoint as Yo, SOLANA_ERROR__JSON_RPC__INTERNAL_ERROR as Yp, singleTransactionPlan as Yr, getConstantEncoder as Ys, assertIsOffchainMessageContentRestrictedAsciiOf1232BytesMax as Yt, mergeBytes as Yu, isTransactionSigner as Z, isStringifiedNumber as Za, getI16Encoder as Zc, SOLANA_ERROR__CODECS__INVALID_LITERAL_UNION_VARIANT as Zd, SOLANA_ERROR__INSTRUCTION_ERROR__INVALID_ACCOUNT_DATA as Zf, SOLANA_ERROR__TRANSACTION__MALFORMED_MESSAGE_BYTES as Zg, SOLANA_ERROR__TRANSACTION_ERROR__INVALID_ACCOUNT_FOR_FEE as Zh, assertIsTransactionMessageWithDurableNonceLifetime as Zi, parseBase58RpcAccount as Zl, SOLANA_ERROR__PROGRAM_CLIENTS__FAILED_TO_IDENTIFY_ACCOUNT as Zm, createFailedToSendTransactionError as Zn, eqBinaryFixedPoint as Zo, SOLANA_ERROR__JSON_RPC__INVALID_REQUEST as Zp, successfulSingleTransactionPlanResultFromTransaction as Zr, getDiscriminatedUnionDecoder as Zs, assertIsOffchainMessageContentUtf8Of65535BytesMax as Zt, offsetDecoder as Zu, getInnerInstructionsFromMeta as _, isSolanaError as __, prependTransactionMessageInstructions as _a, getPatternMatchCodec as _c, SOLANA_ERROR__ACCOUNTS__EXPECTED_ALL_ACCOUNTS_TO_BE_DECODED as _d, SOLANA_ERROR__FIXED_POINTS__INVALID_ZERO_DENOMINATOR_RATIO as _f, SOLANA_ERROR__TRANSACTION_ERROR__WOULD_EXCEED_ACCOUNT_DATA_BLOCK_LIMIT as _g, SOLANA_ERROR__SIGNER__EXPECTED_KEY_PAIR_SIGNER as _h, getTransactionLifetimeConstraintFromCompiledTransactionMessage as _i, getU32Decoder as _l, SOLANA_ERROR__JSON_RPC__SERVER_ERROR_TRANSACTION_PRECOMPILE_VERIFICATION_FAILURE as _m, getOffchainMessageV1Decoder as _n, isSignerRole as _o, SOLANA_ERROR__INSTRUCTION_ERROR__READONLY_LAMPORT_CHANGE as _p, isInstructionPlan as _r, multiplyBinaryFixedPoint as _s, getRpcSubscriptionsChannelWithAutoping as _t, getBaseXResliceDecoder as _u, estimateAndSetComputeUnitLimitFactory as a, SOLANA_ERROR__TRANSACTION__TOO_MANY_SIGNER_ADDRESSES as a_, getTransactionMessageComputeUnitLimit as aa, getHiddenSuffixCodec as ac, padRightEncoder as ad, SOLANA_ERROR__CODECS__SENTINEL_MISSING_IN_DECODED_BYTES as af, SOLANA_ERROR__TRANSACTION_ERROR__INVALID_WRITABLE_ACCOUNT as ag, SOLANA_ERROR__REACT__MISSING_PROVIDER as ah, assertIsFullySignedTransaction as ai, getI8Decoder as al, SOLANA_ERROR__JSON_RPC__SERVER_ERROR_FILTER_TRANSACTION_NOT_FOUND as am, getOffchainMessageApplicationDomainDecoder as an, stringifiedNumber as ao, SOLANA_ERROR__INSTRUCTION_ERROR__MAX_ACCOUNTS_EXCEEDED as ap, findInstructionPlan as ar, getDecimalFixedPointDecoder as as, signTransactionMessageWithSigners as at, getBase16Decoder$1 as au, addSignersToInstruction as b, setTransactionMessageFeePayer as ba, getPredicateCodec as bc, SOLANA_ERROR__ACCOUNTS__ONE_OR_MORE_ACCOUNTS_NOT_FOUND as bd, SOLANA_ERROR__FIXED_POINTS__STRICT_MODE_PRECISION_LOSS as bf, SOLANA_ERROR__TRANSACTION_ERROR__WOULD_EXCEED_MAX_BLOCK_COST_LIMIT as bg, SOLANA_ERROR__SIGNER__EXPECTED_MESSAGE_SIGNER as bh, getTransactionSize as bi, getU64Decoder as bl, SOLANA_ERROR__JSON_RPC__SERVER_ERROR_UNSUPPORTED_TRANSACTION_VERSION as bm, isOffchainMessageApplicationDomain as bn, upgradeRoleToSigner as bo, SOLANA_ERROR__INSTRUCTION_ERROR__UNBALANCED_INSTRUCTION as bp, isNonDivisibleSequentialTransactionPlan as br, negateDecimalFixedPoint as bs, getRpcSubscriptionsTransportWithSubscriptionCoalescing as bt, getUtf8Decoder as bu, estimateResourceLimitsFactory as c, SOLANA_ERROR__TRANSACTION__VERSION_ZERO_MUST_BE_ENCODED_WITH_SIGNATURES_FIRST as c_, getTransactionMessageLoadedAccountsDataSizeLimit as ca, getLiteralUnionCodec as cc, resizeEncoder as cd, SOLANA_ERROR__FAILED_TO_SEND_TRANSACTION as cf, SOLANA_ERROR__TRANSACTION_ERROR__PROGRAM_ACCOUNT_NOT_FOUND as cg, SOLANA_ERROR__RPC_SUBSCRIPTIONS__CHANNEL_CLOSED_BEFORE_MESSAGE_BUFFERED as ch, assertIsTransactionWithBlockhashLifetime as ci, getShortU16Decoder as cl, SOLANA_ERROR__JSON_RPC__SERVER_ERROR_LONG_TERM_STORAGE_UNREACHABLE as cm, getOffchainMessageDecoder as cn, AccountRole as co, SOLANA_ERROR__INSTRUCTION_ERROR__MISSING_ACCOUNT as cp, flattenInstructionPlan as cr, gtDecimalFixedPoint as cs, DEFAULT_RPC_SUBSCRIPTIONS_CONFIG as ct, getBase58Decoder$1 as cu, fillTransactionMessageProvisoryResourceLimits as d, SOLANA_ERROR__WALLET__NO_SIGNER_CONNECTED as d_, getTransactionVersionDecoder as da, getMapCodec as dc, reverseEncoder as dd, SOLANA_ERROR__FIXED_POINTS__DIVISION_BY_ZERO as df, SOLANA_ERROR__TRANSACTION_ERROR__SANITIZE_FAILURE as dg, SOLANA_ERROR__RPC_SUBSCRIPTIONS__EXPECTED_SERVER_SUBSCRIPTION_ID as dh, compileTransaction as di, getU128Decoder as dl, SOLANA_ERROR__JSON_RPC__SERVER_ERROR_NO_SLOT_HISTORY as dm, getOffchainMessageEnvelopeDecoder as dn, assertIsInstructionWithData as do, SOLANA_ERROR__INSTRUCTION_ERROR__NOT_ENOUGH_ACCOUNT_KEYS as dp, getFirstFailedSingleTransactionPlanResult as dr, isBinaryFixedPoint as ds, createDefaultSolanaRpcSubscriptionsChannelCreator as dt, getBase64Decoder as du, SOLANA_ERROR__TRANSACTION__SIGNATURES_MISSING as e_, createTransactionMessage as ea, getEnumDecoder as ec, padLeftCodec as ed, SOLANA_ERROR__CODECS__INVALID_PATTERN_MATCH_VALUE as ef, SOLANA_ERROR__TRANSACTION_ERROR__INVALID_ADDRESS_LOOKUP_TABLE_INDEX as eg, SOLANA_ERROR__PROGRAM_CLIENTS__RESOLVED_INSTRUCTION_INPUT_MUST_BE_NON_NULL as eh, transformTransactionPlan as ei, getI32Encoder as el, SOLANA_ERROR__JSON_RPC__SCAN_ERROR as em, assertIsOffchainMessageUtf8Of65535BytesMax as en, lamportsToSol as eo, SOLANA_ERROR__INSTRUCTION_ERROR__INVALID_ERROR as ep, createTransactionPlanner as er, formatDecimalFixedPoint as es, partiallySignTransactionWithSigners as et, assertValidBaseString$1 as eu, sendAndConfirmDurableNonceTransactionFactory as f, SOLANA_ERROR__WALLET__SIGNER_NOT_AVAILABLE as f_, getTransactionVersionEncoder as fa, getMapDecoder as fc, toArrayBuffer as fd, SOLANA_ERROR__FIXED_POINTS__FRACTIONAL_BITS_EXCEED_TOTAL_BITS as ff, SOLANA_ERROR__TRANSACTION_ERROR__SIGNATURE_FAILURE as fg, SOLANA_ERROR__RPC__API_PLAN_MISSING_FOR_RPC_METHOD as fh, getBase64EncodedWireTransaction as fi, getU128Encoder as fl, SOLANA_ERROR__JSON_RPC__SERVER_ERROR_NO_SNAPSHOT as fm, getOffchainMessageEnvelopeEncoder as fn, downgradeRoleToNonSigner as fo, SOLANA_ERROR__INSTRUCTION_ERROR__PRIVILEGE_ESCALATION as fp, getLinearMessagePackerInstructionPlan as fr, isDecimalFixedPoint as fs, createRpcSubscriptionsTransportFromChannelCreator as ft, getBase64Encoder as fu, getAccountMetasFromCompiledTransactionMessage as g, getSolanaErrorFromTransactionError as g_, prependTransactionMessageInstruction as ga, getNullableEncoder as gc, SOLANA_ERROR__ACCOUNTS__ACCOUNT_NOT_FOUND as gd, SOLANA_ERROR__FIXED_POINTS__INVALID_TOTAL_BITS as gf, SOLANA_ERROR__TRANSACTION_ERROR__UNSUPPORTED_VERSION as gg, SOLANA_ERROR__SIGNER__ADDRESS_CANNOT_HAVE_MULTIPLE_SIGNERS as gh, getTransactionEncoder as gi, getU32Codec as gl, SOLANA_ERROR__JSON_RPC__SERVER_ERROR_TRANSACTION_HISTORY_NOT_AVAILABLE as gm, getOffchainMessageV1Codec as gn, isInstructionWithData as go, SOLANA_ERROR__INSTRUCTION_ERROR__READONLY_DATA_MODIFIED as gp, isFailedSingleTransactionPlanResult as gr, lteDecimalFixedPoint as gs, getChannelPoolingChannelCreator as gt, getBaseXResliceCodec as gu, decodeTransactionFromRpcResponse as h, getSolanaErrorFromJsonRpcError as h_, isTransactionMessageWithDurableNonceLifetime as ha, getNullableDecoder as hc, transformEncoder as hd, SOLANA_ERROR__FIXED_POINTS__INVALID_STRING as hf, SOLANA_ERROR__TRANSACTION_ERROR__UNKNOWN as hg, SOLANA_ERROR__RPC__TRANSPORT_HTTP_HEADER_FORBIDDEN as hh, getTransactionDecoder as hi, getU16Encoder as hl, SOLANA_ERROR__JSON_RPC__SERVER_ERROR_SLOT_SKIPPED as hm, getOffchainMessageV0Encoder as hn, isInstructionWithAccounts as ho, SOLANA_ERROR__INSTRUCTION_ERROR__PROGRAM_FAILED_TO_COMPLETE as hp, isCanceledSingleTransactionPlanResult as hr, lteBinaryFixedPoint as hs, createSolanaRpcSubscriptions_UNSTABLE as ht, getBaseXEncoder$1 as hu, decompileTransactionMessageFetchingLookupTables as i, SOLANA_ERROR__TRANSACTION__TOO_MANY_INSTRUCTIONS as i_, getCompiledTransactionMessageEncoder as ia, getHiddenPrefixEncoder as ic, padRightDecoder as id, SOLANA_ERROR__CODECS__OFFSET_OUT_OF_RANGE as if, SOLANA_ERROR__TRANSACTION_ERROR__INVALID_RENT_PAYING_ACCOUNT as ig, SOLANA_ERROR__REACT__MISSING_CAPABILITY as ih, TRANSACTION_SIZE_LIMIT as ii, getI8Codec as il, SOLANA_ERROR__JSON_RPC__SERVER_ERROR_EPOCH_REWARDS_PERIOD_ACTIVE as im, getOffchainMessageApplicationDomainCodec as in, stringifiedBigInt as io, SOLANA_ERROR__INSTRUCTION_ERROR__MAX_ACCOUNTS_DATA_ALLOCATIONS_EXCEEDED as ip, failedSingleTransactionPlanResult as ir, getDecimalFixedPointCodec as is, signOffchainMessageWithSigners as it, getBase16Codec as iu, assertIsTransactionSigner as j, blockhash as ja, getTupleEncoder as jc, SOLANA_ERROR__ADDRESSES__PDA_ENDS_WITH_PDA_MARKER as jd, SOLANA_ERROR__INSTRUCTION_ERROR__ARITHMETIC_OVERFLOW as jf, SOLANA_ERROR__TRANSACTION__EXPECTED_NONCE_LIFETIME as jg, SOLANA_ERROR__SUBTLE_CRYPTO__DIGEST_UNIMPLEMENTED as jh, assertIsSignatureBytes as ji, getAddressComparator as jl, SOLANA_ERROR__MALFORMED_NUMBER_STRING as jm, appendTransactionMessageInstructionPlan as jn, unwrapOption as jo, SOLANA_ERROR__INSTRUCTION_PLANS__MESSAGE_CANNOT_ACCOMMODATE_PLAN as jp, isSuccessfulSingleTransactionPlanResult as jr, toUnsignedBinaryFixedPoint as js, createSolanaRpcFromTransport as jt, assertByteArrayIsNotEmptyForCodec as ju, assertIsTransactionPartialSigner as k, assertIsStringifiedNumber as ka, getTupleCodec as kc, SOLANA_ERROR__ADDRESSES__MAX_PDA_SEED_LENGTH_EXCEEDED as kd, SOLANA_ERROR__INSTRUCTION_ERROR__ACCOUNT_NOT_EXECUTABLE as kf, SOLANA_ERROR__TRANSACTION__EXCEEDS_SIZE_LIMIT as kg, SOLANA_ERROR__SUBSCRIBABLE__RETRY_NOT_SUPPORTED as kh, signTransaction as ki, createAddressWithSeed as kl, SOLANA_ERROR__MALFORMED_BIGINT_STRING as km, signOffchainMessageEnvelope as kn, none as ko, SOLANA_ERROR__INSTRUCTION_PLANS__INVALID_MAX_INSTRUCTIONS_PER_TRANSACTION as kp, isSingleTransactionPlan as kr, toSignedBinaryFixedPoint as ks, createDefaultRpcTransport as kt, addEncoderSizePrefix as ku, fetchAddressesForLookupTables as l, SOLANA_ERROR__WALLET__ACCOUNT_NOT_AVAILABLE as l_, getTransactionMessagePriorityFeeLamports as la, getLiteralUnionDecoder as lc, reverseCodec as ld, SOLANA_ERROR__FAILED_TO_SEND_TRANSACTIONS as lf, SOLANA_ERROR__TRANSACTION_ERROR__PROGRAM_EXECUTION_TEMPORARILY_RESTRICTED as lg, SOLANA_ERROR__RPC_SUBSCRIPTIONS__CHANNEL_CONNECTION_CLOSED as lh, assertIsTransactionWithDurableNonceLifetime as li, getShortU16Encoder as ll, SOLANA_ERROR__JSON_RPC__SERVER_ERROR_MIN_CONTEXT_SLOT_NOT_REACHED as lm, getOffchainMessageEncoder as ln, assertIsInstructionForProgram as lo, SOLANA_ERROR__INSTRUCTION_ERROR__MISSING_REQUIRED_SIGNATURE as lp, flattenTransactionPlan as lr, gteBinaryFixedPoint as ls, createDefaultRpcSubscriptionsChannelCreator as lt, getBase58Encoder$1 as lu, sendTransactionWithoutConfirmingFactory as m, getSolanaErrorFromInstructionError as m_, isTransactionMessageWithBlockhashLifetime as ma, getNullableCodec as mc, transformDecoder as md, SOLANA_ERROR__FIXED_POINTS__INVALID_FRACTIONAL_BITS as mf, SOLANA_ERROR__TRANSACTION_ERROR__UNBALANCED_TRANSACTION as mg, SOLANA_ERROR__RPC__TRANSPORT_HTTP_ERROR as mh, getTransactionCodec as mi, getU16Decoder as ml, SOLANA_ERROR__JSON_RPC__SERVER_ERROR_SLOT_NOT_EPOCH_BOUNDARY as mm, getOffchainMessageV0Decoder as mn, isInstructionForProgram as mo, SOLANA_ERROR__INSTRUCTION_ERROR__PROGRAM_FAILED_TO_COMPILE as mp, getReallocMessagePackerInstructionPlan as mr, ltDecimalFixedPoint as ms, createSolanaRpcSubscriptionsFromTransport as mt, getBaseXDecoder$1 as mu, createAsyncGeneratorWithInitialValueAndSlotTracking as n, SOLANA_ERROR__TRANSACTION__TOO_MANY_ACCOUNTS_IN_INSTRUCTION as n_, getCompiledTransactionMessageCodec as na, getHiddenPrefixCodec as nc, padLeftEncoder as nd, SOLANA_ERROR__CODECS__LITERAL_UNION_DISCRIMINATOR_OUT_OF_RANGE as nf, SOLANA_ERROR__TRANSACTION_ERROR__INVALID_LOADED_ACCOUNTS_DATA_SIZE_LIMIT as ng, SOLANA_ERROR__PROGRAM_CLIENTS__UNRECOGNIZED_ACCOUNT_TYPE as nh, TRANSACTION_PACKET_HEADER as ni, getI64Decoder as nl, SOLANA_ERROR__JSON_RPC__SERVER_ERROR_BLOCK_NOT_AVAILABLE as nm, compileOffchainMessageV0Envelope as nn, sol as no, SOLANA_ERROR__INSTRUCTION_ERROR__INVALID_REALLOC as np, everyTransactionPlan as nr, getBinaryFixedPointDecoder as ns, signAndSendTransactionMessageWithSigners as nt, getBase10Decoder as nu, estimateAndSetResourceLimitsFactory as o, SOLANA_ERROR__TRANSACTION__VERSION_NUMBER_NOT_SUPPORTED as o_, getTransactionMessageComputeUnitPrice as oa, getHiddenSuffixDecoder as oc, resizeCodec as od, SOLANA_ERROR__CODECS__UNION_VARIANT_OUT_OF_RANGE as of, SOLANA_ERROR__TRANSACTION_ERROR__MAX_LOADED_ACCOUNTS_DATA_SIZE_EXCEEDED as og, SOLANA_ERROR__REACT__SUBSCRIPTION_CLOSED_WITHOUT_ERROR as oh, assertIsSendableTransaction as oi, getI8Encoder as ol, SOLANA_ERROR__JSON_RPC__SERVER_ERROR_KEY_EXCLUDED_FROM_SECONDARY_INDEX as om, getOffchainMessageApplicationDomainEncoder as on, testnet as oo, SOLANA_ERROR__INSTRUCTION_ERROR__MAX_INSTRUCTION_TRACE_LENGTH_EXCEEDED as op, findTransactionPlan as or, getDecimalFixedPointEncoder as os, signTransactionWithSigners as ot, getBase16Encoder as ou, sendAndConfirmTransactionFactory as p, SolanaError as p_, isAdvanceNonceAccountInstruction as pa, getMapEncoder as pc, transformCodec as pd, SOLANA_ERROR__FIXED_POINTS__INVALID_DECIMALS as pf, SOLANA_ERROR__TRANSACTION_ERROR__TOO_MANY_ACCOUNT_LOCKS as pg, SOLANA_ERROR__RPC__INTEGER_OVERFLOW as ph, getSignatureFromTransaction as pi, getU16Codec as pl, SOLANA_ERROR__JSON_RPC__SERVER_ERROR_SEND_TRANSACTION_PREFLIGHT_FAILURE as pm, getOffchainMessageV0Codec as pn, downgradeRoleToReadonly as po, SOLANA_ERROR__INSTRUCTION_ERROR__PROGRAM_ENVIRONMENT_SETUP_FAILURE as pp, getMessagePackerInstructionPlanFromInstructions as pr, ltBinaryFixedPoint as ps, createSolanaRpcSubscriptions as pt, getBaseXCodec as pu, isTransactionMessageWithSingleSendingSigner as q, isBlockhash as qa, getI128Decoder as qc, SOLANA_ERROR__CODECS__INVALID_BYTE_LENGTH as qd, SOLANA_ERROR__INSTRUCTION_ERROR__IMMUTABLE as qf, SOLANA_ERROR__TRANSACTION__INVALID_NONCE_TRANSACTION_FIRST_INSTRUCTION_MUST_BE_ADVANCE_NONCE as qg, SOLANA_ERROR__TRANSACTION_ERROR__CLUSTER_MAINTENANCE as qh, MAX_SUPPORTED_TRANSACTION_VERSION as qi, fetchEncodedAccount as ql, SOLANA_ERROR__OFFCHAIN_MESSAGE__SIGNATURES_MISSING as qm, assertIsSuccessfulSingleTransactionPlanResult as qn, decimalFixedPointToNumber as qo, SOLANA_ERROR__INVARIANT_VIOLATION__SUBSCRIPTION_ITERATOR_STATE_MISSING as qp, sequentialTransactionPlanResult as qr, getConstantCodec as qs, assertIsFullySignedOffchainMessageEnvelope as qt, isFixedSize as qu, createReactiveStoreWithInitialValueAndSlotTracking as r, SOLANA_ERROR__TRANSACTION__TOO_MANY_ACCOUNT_ADDRESSES as r_, getCompiledTransactionMessageDecoder as ra, getHiddenPrefixDecoder as rc, padRightCodec as rd, SOLANA_ERROR__CODECS__NUMBER_OUT_OF_RANGE as rf, SOLANA_ERROR__TRANSACTION_ERROR__INVALID_PROGRAM_FOR_EXECUTION as rg, SOLANA_ERROR__PROGRAM_CLIENTS__UNRECOGNIZED_INSTRUCTION_TYPE as rh, TRANSACTION_PACKET_SIZE as ri, getI64Encoder as rl, SOLANA_ERROR__JSON_RPC__SERVER_ERROR_BLOCK_STATUS_NOT_AVAILABLE_YET as rm, compileOffchainMessageV1Envelope as rn, solToLamports as ro, SOLANA_ERROR__INSTRUCTION_ERROR__INVALID_SEEDS as rp, everyTransactionPlanResult as rr, getBinaryFixedPointEncoder as rs, signAndSendTransactionWithSigners as rt, getBase10Encoder as ru, estimateComputeUnitLimitFactory as s, SOLANA_ERROR__TRANSACTION__VERSION_NUMBER_OUT_OF_RANGE as s_, getTransactionMessageHeapSize as sa, getHiddenSuffixEncoder as sc, resizeDecoder as sd, SOLANA_ERROR__CRYPTO__RANDOM_VALUES_FUNCTION_UNIMPLEMENTED as sf, SOLANA_ERROR__TRANSACTION_ERROR__MISSING_SIGNATURE_FOR_FEE as sg, SOLANA_ERROR__RPC_SUBSCRIPTIONS__CANNOT_CREATE_SUBSCRIPTION_PLAN as sh, assertIsTransactionMessageWithinSizeLimit as si, getShortU16Codec as sl, SOLANA_ERROR__JSON_RPC__SERVER_ERROR_LONG_TERM_STORAGE_SLOT_SKIPPED as sm, getOffchainMessageCodec as sn, unixTimestamp as so, SOLANA_ERROR__INSTRUCTION_ERROR__MAX_SEED_LENGTH_EXCEEDED as sp, findTransactionPlanResult as sr, gtBinaryFixedPoint as ss, writeKeyPairSigner as st, getBase58Codec as su, airdropFactory as t, SOLANA_ERROR__TRANSACTION__SIGNATURE_COUNT_TOO_HIGH_FOR_TRANSACTION_BYTES as t_, decompileTransactionMessage4 as ta, getEnumEncoder as tc, padLeftDecoder as td, SOLANA_ERROR__CODECS__INVALID_STRING_FOR_BASE as tf, SOLANA_ERROR__TRANSACTION_ERROR__INVALID_ADDRESS_LOOKUP_TABLE_OWNER as tg, SOLANA_ERROR__PROGRAM_CLIENTS__UNEXPECTED_RESOLVED_INSTRUCTION_INPUT_TYPE as th, transformTransactionPlanResult as ti, getI64Codec as tl, SOLANA_ERROR__JSON_RPC__SERVER_ERROR_BLOCK_CLEANED_UP as tm, compileOffchainMessageEnvelope as tn, mainnet as to, SOLANA_ERROR__INSTRUCTION_ERROR__INVALID_INSTRUCTION_DATA as tp, everyInstructionPlan as tr, getBinaryFixedPointCodec as ts, setTransactionMessageFeePayerSigner as tt, getBase10Codec as tu, fillTransactionMessageProvisoryComputeUnitLimit as u, SOLANA_ERROR__WALLET__NOT_CONNECTED as u_, getTransactionVersionCodec as ua, getLiteralUnionEncoder as uc, reverseDecoder as ud, SOLANA_ERROR__FIXED_POINTS__ARITHMETIC_OVERFLOW as uf, SOLANA_ERROR__TRANSACTION_ERROR__RESANITIZATION_NEEDED as ug, SOLANA_ERROR__RPC_SUBSCRIPTIONS__CHANNEL_FAILED_TO_CONNECT as uh, assertIsTransactionWithinSizeLimit as ui, getU128Codec as ul, SOLANA_ERROR__JSON_RPC__SERVER_ERROR_NODE_UNHEALTHY as um, getOffchainMessageEnvelopeCodec as un, assertIsInstructionWithAccounts as uo, SOLANA_ERROR__INSTRUCTION_ERROR__MODIFIED_PROGRAM_ID as up, flattenTransactionPlanResult as ur, gteDecimalFixedPoint as us, createDefaultRpcSubscriptionsTransport as ut, getBase64Codec as uu, getInstructionsFromCompiledTransactionMessage as v, safeCaptureStackTrace as v_, setTransactionMessageComputeUnitLimit as va, getPatternMatchDecoder as vc, SOLANA_ERROR__ACCOUNTS__EXPECTED_DECODED_ACCOUNT as vd, SOLANA_ERROR__FIXED_POINTS__MALFORMED_RAW_VALUE as vf, SOLANA_ERROR__TRANSACTION_ERROR__WOULD_EXCEED_ACCOUNT_DATA_TOTAL_LIMIT as vg, SOLANA_ERROR__SIGNER__EXPECTED_MESSAGE_MODIFYING_SIGNER as vh, getTransactionMessageSize as vi, getU32Encoder as vl, SOLANA_ERROR__JSON_RPC__SERVER_ERROR_TRANSACTION_SIGNATURE_LEN_MISMATCH as vm, getOffchainMessageV1Encoder as vn, isWritableRole as vo, SOLANA_ERROR__INSTRUCTION_ERROR__REENTRANCY_NOT_ALLOWED as vp, isMessagePackerInstructionPlan as vr, multiplyDecimalFixedPoint as vs, getRpcSubscriptionsChannelWithBigIntJSONSerialization as vt, getBaseXResliceEncoder as vu, assertIsMessageModifyingSigner as w, setTransactionMessageLoadedAccountsDataSizeLimit as wa, getSetDecoder as wc, SOLANA_ERROR__ADDRESSES__INVALID_ED25519_PUBLIC_KEY as wd, SOLANA_ERROR__INSTRUCTION_ERROR__ACCOUNT_ALREADY_INITIALIZED as wf, SOLANA_ERROR__TRANSACTION__ADDRESSES_CANNOT_SIGN_TRANSACTION as wg, SOLANA_ERROR__SIGNER__EXPECTED_TRANSACTION_SIGNER as wh, isTransactionMessageWithinSizeLimit as wi, getU8Encoder as wl, SOLANA_ERROR__KEYS__INVALID_SIGNATURE_BYTE_LENGTH as wm, offchainMessageApplicationDomain as wn, getOptionDecoder as wo, SOLANA_ERROR__INSTRUCTION_ERROR__UNSUPPORTED_SYSVAR as wp, isParallelTransactionPlanResult as wr, rawDecimalFixedPoint as ws, createSubscriptionRpc as wt, addCodecSentinel as wu, addSignersToTransactionMessage as x, setTransactionMessageHeapSize as xa, getPredicateDecoder as xc, SOLANA_ERROR__ADDRESSES__FAILED_TO_FIND_VIABLE_PDA_BUMP_SEED as xd, SOLANA_ERROR__FIXED_POINTS__TOTAL_BITS_NOT_BYTE_ALIGNED as xf, SOLANA_ERROR__TRANSACTION_ERROR__WOULD_EXCEED_MAX_VOTE_COST_LIMIT as xg, SOLANA_ERROR__SIGNER__EXPECTED_TRANSACTION_MODIFYING_SIGNER as xh, getTransactionSizeLimit as xi, getU64Encoder as xl, SOLANA_ERROR__KEYS__INVALID_BASE58_IN_GRIND_REGEX as xm, isOffchainMessageContentRestrictedAsciiOf1232BytesMax as xn, upgradeRoleToWritable as xo, SOLANA_ERROR__INSTRUCTION_ERROR__UNINITIALIZED_ACCOUNT as xp, isNonDivisibleSequentialTransactionPlanResult as xr, ratioBinaryFixedPoint as xs, createSolanaRpcSubscriptionsApi as xt, getUtf8Encoder as xu, walkInstructions as y, unwrapSimulationError as y_, setTransactionMessageComputeUnitPrice as ya, getPatternMatchEncoder as yc, SOLANA_ERROR__ACCOUNTS__FAILED_TO_DECODE_ACCOUNT as yd, SOLANA_ERROR__FIXED_POINTS__SHAPE_MISMATCH as yf, SOLANA_ERROR__TRANSACTION_ERROR__WOULD_EXCEED_MAX_ACCOUNT_COST_LIMIT as yg, SOLANA_ERROR__SIGNER__EXPECTED_MESSAGE_PARTIAL_SIGNER as yh, getTransactionMessageSizeLimit as yi, getU64Codec as yl, SOLANA_ERROR__JSON_RPC__SERVER_ERROR_TRANSACTION_SIGNATURE_VERIFICATION_FAILURE as ym, isFullySignedOffchainMessageEnvelope as yn, mergeRoles as yo, SOLANA_ERROR__INSTRUCTION_ERROR__RENT_EPOCH_MODIFIED as yp, isNonDivisibleSequentialInstructionPlan as yr, negateBinaryFixedPoint as ys, getRpcSubscriptionsChannelWithJSONSerialization as yt, getUtf8Codec as yu, getSignersFromOffchainMessage as z, getDefaultLamportsDecoder as za, assertNumberIsBetweenForCodec as zc, SOLANA_ERROR__CODECS__ENCODER_DECODER_MAX_SIZE_MISMATCH as zd, SOLANA_ERROR__INSTRUCTION_ERROR__EXECUTABLE_ACCOUNT_NOT_RENT_EXEMPT as zf, SOLANA_ERROR__TRANSACTION__FAILED_WHEN_SIMULATING_TO_ESTIMATE_COMPUTE_LIMIT as zg, SOLANA_ERROR__TRANSACTION_ERROR__ACCOUNT_BORROW_OUTSTANDING as zh, isSignature as zi, isProgramDerivedAddress as zl, SOLANA_ERROR__OFFCHAIN_MESSAGE__MESSAGE_LENGTH_MISMATCH as zm, assertIsParallelTransactionPlan as zn, assertIsDecimalFixedPoint as zo, SOLANA_ERROR__INSTRUCTION__PROGRAM_ID_MISMATCH as zp, parallelTransactionPlan as zr, getBitArrayDecoder as zs, createReactiveStoreFromDataPublisherFactory as zt, createDecoder as zu };
